...
public class BirtFactory {
    ...
    public static String getReportDocumentPath(String resourceName) throws IOException {
        ...
        synchronized (BirtFactory.class) {
            ...
            URL reportDocumentUrl = null;
            reportDocumentUrl = FlexibleLocation.resolveLocation(resourceName, loader);
            if (reportDocumentUrl == null) {
                throw new IllegalArgumentException("Could not resolve location to URL: " + resourceName);
            }
            returnPath = reportDocumentUrl.getPath();
            double totalSeconds = (System.currentTimeMillis() - startTime)/1000.0;
            Debug.logInfo("Got report in " + totalSeconds + "s from: " + reportDocumentUrl.toExternalForm(), module);
        }

        if (returnPath == null) {
            throw new IllegalArgumentException("Could not find report document with location [" + resourceName + "]");
        }
        return returnPath;
    }
}
