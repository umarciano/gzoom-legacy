...
//				design = engine.openReportDesign(servletContext.getRealPath(page));
				reportUrl = servletContext.getRealPath(page);
			}

			BirtWorker.setWebContextObjects(engine.getConfig().getAppContext(), request, response);
			...
			if (UtilValidate.isNotEmpty(timezone))
				parameters.put("timeZone", timezone.getID());
