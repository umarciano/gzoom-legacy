...
public class TransactionUtil implements Status {
    ...
    public static boolean debugResources = false;
        ...
        if (ut != null) {
            try {
                ...
                if (timeout > 0 && timeout > Integer.MAX_VALUE / 1000) {
                    Debug.logVerbose("[TransactionUtil.begin] timeout : " + timeout + " seconds exceed Integer.MAX_VALUE / 1000 so set to " + Integer.MAX_VALUE / 1000, module);
                    timeout = Integer.MAX_VALUE / 1000;
                }
                
                // set the timeout for THIS transaction
                if (timeout > 0) {
                    ut.setTransactionTimeout(timeout);
                    Debug.logTiming("[TransactionUtil.begin] set transaction timeout to : " + timeout + " seconds", module);
                }

                // begin the transaction
                ut.begin();
                Debug.logTiming("[TransactionUtil.begin] transaction begin", module);
        ...
        if (ut != null) {
            try {
                ...
                if (status != STATUS_NO_TRANSACTION && status != STATUS_COMMITTING && status != STATUS_COMMITTED && status != STATUS_ROLLING_BACK && status != STATUS_ROLLEDBACK) {
                    ...
                    Debug.logTiming("[TransactionUtil.commit] transaction committed", module);
                } else {
