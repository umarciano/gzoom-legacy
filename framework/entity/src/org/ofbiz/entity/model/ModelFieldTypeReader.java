...
public class ModelFieldTypeReader implements Serializable {
    ...
    public static ModelFieldTypeReader getModelFieldTypeReader(String helperName) {
        ...
        if (reader == null) {
            synchronized (readers) {
                ...
                initializeFieldTypes(datasourceInfo, reader);
    ...
    protected static void initializeFieldTypes(DatasourceInfo datasourceInfo, ModelFieldTypeReader reader) {
        ClassLoader loader = Thread.currentThread().getContextClassLoader();
        Collection<ModelFieldType> fieldTypes = reader.getFieldTypes();
        for (ModelFieldType modelFieldType : fieldTypes) {
            String javaType = modelFieldType.getJavaType();
            Class<?> javaClass = null;
            try {
                javaClass = loader.loadClass(javaType);
            } catch (ClassNotFoundException e) {
                Debug.logError(e, module);
            }
            if (javaClass == null) {
                Debug.logWarning("Java type '" + javaType + "' for " + modelFieldType.getType() + 
                        " not found in fieldtype" + datasourceInfo.fieldTypeName + 
                        ". Fields using this type will be ignored.", module);
                continue;
            }
            String jdbcDataType = modelFieldType.getJdbcDataType();
            if (jdbcDataType.length() == 0) {
                jdbcDataType = modelFieldType.getSqlType();
                int pos = jdbcDataType.indexOf("(");
                if (pos != -1) {
                    jdbcDataType = jdbcDataType.substring(0, pos);
                }
            }
            SqlObjectHandler handler = null;
            try {
                handler = SqlTypes.toSqlObjectHandler(jdbcDataType, javaClass);
            } catch (ClassNotFoundException e) {
                Debug.logError(e, module);
            }
            if (handler == null) {
                // Try using java-type
                jdbcDataType = javaClass.getSimpleName().toUpperCase();
                try {
                    handler = SqlTypes.toSqlObjectHandler(jdbcDataType, javaClass);
                } catch (ClassNotFoundException e) {
                    Debug.logError(e, module);
            ...
            if (handler != null) {
                modelFieldType.setSqlObjectHandler(handler);
            } else {
                Debug.logWarning("jdbc-data-type '" + jdbcDataType + "' for " + modelFieldType.getType() + 
                        " not found in fieldtype" + datasourceInfo.fieldTypeName + 
                        ". Fields using this type will be ignored.", module);
            }
        }
