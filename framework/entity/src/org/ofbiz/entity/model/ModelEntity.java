...
public class ModelEntity extends ModelInfo implements Comparable<ModelEntity>, Serializable {
    ...
    public static final String CREATE_USER_LOGIN_ID_FIELD = "createdByUserLogin";
    public static final String USER_LOGIN_ID_FIELD = "lastModifiedByUserLogin";
    ...
    /** Can be used to disable automatically creating user login fields and populating them on inserts and updates */
    protected boolean noAutoLogin = true;
    ...
    public ModelEntity(ModelReader reader, Element entityElement, UtilTimer utilTimer, ModelInfo def) {
        ...
        this.populateAutoLogin(reader);
        this.populateAutoTimestamp(reader);

        if (utilTimer != null) utilTimer.timerString("  createModelEntity: before prim-keys");
        this.populatePk(reader, entityElement);
    ...
    protected void populateBasicInfo(Element entityElement) {
        ...
        this.noAutoLogin = UtilXml.checkBoolean(entityElement.getAttribute("no-auto-user-log"), true);
    ...
    protected void populatePk(ModelReader reader, Element entityElement) {
    ...
    protected void populateAutoLogin(ModelReader reader) {
        if (!this.noAutoLogin && !this.isField(USER_LOGIN_ID_FIELD)) {
            ModelField newField = reader.createModelField(USER_LOGIN_ID_FIELD, "id-vlong-ne", null, false);
            newField.setIsAutoCreatedInternal(true);
            newField.setModelEntity(this);
            this.fields.add(newField);

            ModelRelation relation = new ModelRelation();
            relation.setType("one-nofk");
            relation.setTitle("LastUpdate");
            relation.setRelEntityName("UserLogin");
            relation.setAutoRelation(true);
            relation.addKeyMap(new ModelKeyMap(USER_LOGIN_ID_FIELD, "userLoginId"));
            this.addRelation(relation);
        }
        if (!this.noAutoLogin && !this.isField(CREATE_USER_LOGIN_ID_FIELD)) {
            ModelField newField = reader.createModelField(CREATE_USER_LOGIN_ID_FIELD, "id-vlong-ne", null, false);
            newField.setIsAutoCreatedInternal(true);
            newField.setModelEntity(this);
            this.fields.add(newField);
            ...
            ModelRelation relation = new ModelRelation();
            relation.setType("one-nofk");
            relation.setTitle("Created");
            relation.setRelEntityName("UserLogin");
            relation.setAutoRelation(true);
            relation.addKeyMap(new ModelKeyMap(CREATE_USER_LOGIN_ID_FIELD, "userLoginId"));
            this.addRelation(relation);
        }
    }

    protected void populateAutoTimestamp(ModelReader reader) {
    ...
    public void addExtendEntity(ModelReader reader, Element extendEntityElement) {
        ...
        this.populatePk(reader, extendEntityElement);
        ...
        this.setNoAutoLogin(UtilXml.checkBoolean(extendEntityElement.getAttribute("no-auto-user-log")));
        this.populateAutoLogin(reader);
        this.setNoAutoStamp(UtilXml.checkBoolean(extendEntityElement.getAttribute("no-auto-stamp")));
        this.populateAutoTimestamp(reader);
    }
    ...
    /**
     * @return Returns the noAutoLogin.
     */
    public boolean getNoAutoLogin() {
        return this.noAutoLogin;
    }

    /**
     * @param noAutoStamp The noAutoLogin to set.
     */
    public void setNoAutoLogin(boolean noAutoLogin) {
