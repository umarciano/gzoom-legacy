...
public class EntityExpr extends EntityCondition {
    ...
    @Override
    public String makeWhereString(ModelEntity modelEntity, List<EntityConditionParam> entityConditionParams, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        ...
        operator.addSqlValue(sql, modelEntity, entityConditionParams, true, lhs, rhs, datasourceInfo, modelFieldTypeReader);
        return sql.toString();
    }
    ...
    public void checkRhsType(ModelEntity modelEntity, Delegator delegator) {
        ...
        if (curField == null) {
            if (UtilValidate.isNotEmpty(entityAlias) && modelEntity instanceof ModelViewEntity) {
                ModelEntity modelMemberEntity = ((ModelViewEntity)modelEntity).getMemberModelEntity(entityAlias);
                curField = modelMemberEntity.getField(fieldName);
            }
            if (curField == null)
        ...
        if (value instanceof EntityConditionSubSelect){
          ...
          // make sure the type of keyFieldName of EntityConditionSubSelect  matches the field Java type
            ...
            EntityFieldValue efv = (EntityFieldValue) value;
            String valueFieldName = efv.getFieldName();
            String valueEntityAlias = efv.entityAlias;
            ModelField valueField = modelEntity.getField(valueFieldName);
            if (valueField == null) {
                if (UtilValidate.isNotEmpty(valueEntityAlias) && modelEntity instanceof ModelViewEntity) {
                    ModelEntity modelMemberEntity = ((ModelViewEntity)modelEntity).getMemberModelEntity(valueEntityAlias);
                    valueField = modelMemberEntity.getField(valueFieldName);
                }
                if (valueField == null) {
                    throw new IllegalArgumentException("FieldName " + valueField + " not found for entity: " + modelEntity.getEntityName());
                }
            }
            ModelFieldType valueType = null;
            try {
                valueType = delegator.getEntityFieldType(modelEntity, valueField.getType());
            } catch (GenericEntityException e) {
//                Debug.logWarning(e, module);
            }
            if (valueType == null) {
                throw new IllegalArgumentException("Type " + valueField.getType() + " not found for entity [" + modelEntity.getEntityName() + "]; probably because there is no datasource (helper) setup for the entity group that this entity is in: [" + delegator.getEntityGroupName(modelEntity.getEntityName()) + "]");
            }
         // make sure the type of value  matches the field Java type
            try {
                if (!ObjectType.instanceOf(ObjectType.loadClass(valueType.getJavaType()), type.getJavaType())) {
//                    String errMsg = "In entity field [" + modelEntity.getEntityName() + "." + valueField.getName() + "] set the value passed in [" + valueType.getJavaType() + "] is not compatible with the Java type of the field [" + type.getJavaType() + "]";
                    // eventually we should do this, but for now we'll do a "soft" failure: throw new IllegalArgumentException(errMsg);
//                    Debug.logWarning(new Exception("Location of database type warning"), "=-=-=-=-=-=-=-=-= Database type warning in EntityExpr =-=-=-=-=-=-=-=-= " + errMsg, module);
                }
