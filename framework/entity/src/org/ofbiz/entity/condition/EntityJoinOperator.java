...
public class EntityJoinOperator extends EntityOperator<EntityCondition, EntityCondition, Boolean> {
    ...
    @Override
    public void addSqlValue(StringBuilder sql, ModelEntity modelEntity, List<EntityConditionParam> entityConditionParams, boolean compat, EntityCondition lhs, EntityCondition rhs, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        sql.append('(');
        sql.append(lhs.makeWhereString(modelEntity, entityConditionParams, datasourceInfo, modelFieldTypeReader));
        sql.append(' ');
        sql.append(getCode());
        sql.append(' ');
        if (rhs instanceof EntityCondition) {
            sql.append(rhs.makeWhereString(modelEntity, entityConditionParams, datasourceInfo, modelFieldTypeReader));
        } else {
    ...
    public void addSqlValue(StringBuilder sql, ModelEntity modelEntity, List<EntityConditionParam> entityConditionParams, List<? extends EntityCondition> conditionList, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        if (UtilValidate.isNotEmpty(conditionList)) {
            ...
            while (conditionIter.hasNext()) {
                ...
                sql.append(condition.makeWhereString(modelEntity, entityConditionParams, datasourceInfo, modelFieldTypeReader));
