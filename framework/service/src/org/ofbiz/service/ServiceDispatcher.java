...
public class ServiceDispatcher {
    ...
    protected ServiceDispatcher(Delegator delegator, boolean enableJM, boolean enableJMS, boolean enableSvcs) {
        ...
        //re-enable job scheduled after EntityDataLoadContainer run
//        ServiceDispatcher.enableJM(true);
//        ServiceDispatcher.enableJMS(true);
//        ServiceDispatcher.enableSvcs(true);
        
        Debug.log("[ServiceDispatcher] : enableJM " + ServiceDispatcher.getEnableJM());
        Debug.log("[ServiceDispatcher] : enableJM param " + enableJM);
    ...
    protected ServiceDispatcher(Delegator delegator) {
        ...
        this(delegator, ServiceDispatcher.getEnableJM(), ServiceDispatcher.getEnableJMS(), ServiceDispatcher.getEnableSvcs());
        Debug.log("parametri senza this"+enableJM +enableJMS + enableSvcs);
        Debug.log("parametri con metodo statico"+ServiceDispatcher.getEnableJM() +ServiceDispatcher.getEnableJMS() + ServiceDispatcher.getEnableSvcs(), module);
    }
    ...
    public Map<String, Object> runSync(String localName, ModelService modelService, Map<String, ? extends Object> params, boolean validateOut) throws ServiceAuthException, ServiceValidationException, GenericServiceException {
        ...
        try {
            if (Debug.timingOn()) {
                Debug.logTiming("=========================== " + modelService.name + " tx status =" + TransactionUtil.getStatusString() + ", modelService.requireNewTransaction=" + modelService.requireNewTransaction + ", modelService.useTransaction=" + modelService.useTransaction + ", modelService.transactionTimeout=" + modelService.transactionTimeout + ", TransactionUtil.isTransactionInPlace()=" + TransactionUtil.isTransactionInPlace(), module);
            }
        ...
        if (Debug.timingOn() && timeToRun > 50) {
            Debug.logTiming("Run sync service [" + localName + "/" + modelService.name + "] finished in [" + timeToRun + "] milliseconds", module);
        } else if (timeToRun > 200) {
            Debug.logInfo("Run sync service [" + localName + "/" + modelService.name + "] finished in [" + timeToRun + "] milliseconds", module);
        }
    ...
    public static boolean getEnableSvcs() {
    	return ServiceDispatcher.enableSvcs;
    }
    
    public static boolean getEnableJM() {
    	return ServiceDispatcher.enableJM;
    }
