...
public class ContextFilter implements Filter {
    ...
    protected String contextFilterAppModule = null;
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        contextFilterAppModule = UtilProperties.getPropertyValue("general.properties", "logMDC.contextFilterAppModule");
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        try {
            Debug.putMDC("appModule", contextFilterAppModule);
            Debug.putMDC("remoteAddr", request.getRemoteAddr());
            HttpServletRequest httpRequest = (HttpServletRequest) request;
            javax.servlet.http.HttpSession session = httpRequest.getSession(false);
            if (session != null) {
                Object obj = session.getAttribute("userLogin");
                if (obj instanceof org.ofbiz.entity.GenericValue) {
                    Debug.putMDC("userLoginId", ((org.ofbiz.entity.GenericValue)obj).get("userLoginId"));
                }
            }
            doFilterPriv(request, response, chain);
        } finally {
            Debug.removeMDC("userLoginId");
            Debug.removeMDC("remoteAddr");
            Debug.removeMDC("appModule");
        }
    }

    private void doFilterPriv(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        HttpServletRequest httpRequest = (HttpServletRequest) request;
