...
public class ModelForm extends ModelWidget {
    ...
    public final String FIELD_ID_SEPARATOR = ".";
    ...
    protected FlexibleStringExpander containerId;
    ...
    protected FlexibleStringExpander pagerPosition;
    ...
    protected FlexibleStringExpander formValidator;
    ...
    protected boolean appendToParentField = true;
    ...
    public void initForm(Element formElement) {
        ...
        if (formElement.hasAttribute("append-field")) {
            String appendField = formElement.getAttribute("append-field");
            if (appendField != null && appendField.equalsIgnoreCase("false"))
                appendToParentField = false;
        }

        List<ModelFormField> parentFieldList = new FastList<ModelFormField>();
        List<ModelFormField> parentMultiSubmitFieldList = new FastList<ModelFormField>();
        ...
        if (parentForm.length() > 0) {
            ...
            if (parent != null) {
                ...
                this.paginateSizeField = parent.paginateSizeField;
                this.paginateIndexField = parent.paginateIndexField;
                ...
                this.pagerPosition = parent.pagerPosition;
                this.readOnlyCondition = parent.readOnlyCondition;
                this.formValidator = parent.formValidator;
                ...
                for (ModelFormField parentChildField: parent.fieldList) {
                    ...
                    if (appendToParentField) {
                        this.fieldList.add(childField);
                    }
                    else {
                        parentFieldList.add(childField);
                    }
                    this.fieldMap.put(childField.getName(), childField);
                }

                if (UtilValidate.isNotEmpty(parent.multiSubmitFields)) {
                    Iterator<ModelFormField> multiSubmitFieldListIter = parent.multiSubmitFields.iterator();
                    while (multiSubmitFieldListIter.hasNext()) {
                        ModelFormField parentChildField = multiSubmitFieldListIter.next();
                        ModelFormField childField = new ModelFormField(this);
                        childField.mergeOverrideModelFormField(parentChildField);
                        if (appendToParentField) {
                            if (UtilValidate.isEmpty(this.multiSubmitFields)){
                                this.multiSubmitFields = new FastList<ModelFormField>();
                            }
                            this.multiSubmitFields.add(childField);
                        }
                        else {
                            parentMultiSubmitFieldList.add(childField);
                        }
                    }
                }
        ...
        if (this.containerId == null || formElement.hasAttribute("id")) {
            this.containerId = FlexibleStringExpander.getInstance(formElement.getAttribute("id"));
        }
        ...
        // pagination settings
        if (this.pagerPosition == null || formElement.hasAttribute("pager-position")) {
            this.pagerPosition = FlexibleStringExpander.getInstance(UtilValidate.isNotEmpty(formElement.getAttribute("pager-position")) ? formElement.getAttribute("pager-position") : "both");
        }
        ...
        if (formElement.hasAttribute("paginate-size")) {
            setDefaultViewSize(formElement.getAttribute("paginate-size"));
        }
        ...
        if (formElement.hasAttribute("form-validator")) {
        	this.formValidator = FlexibleStringExpander.getInstance(formElement.getAttribute("form-validator"));
        }
        ...
        Element readOnlyElement = UtilXml.firstChildElement(formElement, "read-only");
        ...
        if (UtilValidate.isNotEmpty(this.formValidator)) {
        	ModelFormField formValidatorField = this.fieldMap.get("formValidator");
        	if (UtilValidate.isEmpty(formValidatorField)) {
        		formValidatorField = new ModelFormField(this);
        		formValidatorField.setName("formValidator");
        		ModelFormField.HiddenField hiddenFormValidatorField = new ModelFormField.HiddenField(formValidatorField);
        		hiddenFormValidatorField.setValue(this.formValidator.getOriginal());
        		
        		formValidatorField.setFieldInfo(hiddenFormValidatorField);
        	}
        	
        	this.addUpdateField(formValidatorField);
        }
        ...
        if (sortOrderElement != null) {
            ...
            for (Element sortFieldElement: UtilXml.childElementList(sortOrderElement)) {
                ...
                if (tagName.equals("sort-field")) {
                	...
                	lastFieldGroup = new FieldGroup(sortFieldElement, this);
                    this.fieldGroupList.add(lastFieldGroup);
...
//                    this.fieldGroupList.add(lastFieldGroup);
        	...
        	//Nel caso non sia definito un sort-order in questa form tendo ad ereditare
        	this.fieldGroupList = parent.fieldGroupList;
            this.fieldGroupMap = parent.fieldGroupMap;
        }

        if (UtilValidate.isNotEmpty(parentFieldList)) {
            for (ModelFormField field : parentFieldList) {
                if (!this.fieldList.contains(field)) {
                    if (UtilValidate.isEmpty(this.multiSubmitFields) || (UtilValidate.isNotEmpty(this.multiSubmitFields) && !this.multiSubmitFields.contains(field)))
                        this.fieldList.add(field);
                }
            }
        }

        if (UtilValidate.isNotEmpty(parentMultiSubmitFieldList)) {
            for (ModelFormField field : parentMultiSubmitFieldList) {
                if (UtilValidate.isEmpty(this.multiSubmitFields)) {
                    this.multiSubmitFields = new FastList<ModelFormField>();
                }
                if (!this.multiSubmitFields.contains(field))
                    this.multiSubmitFields.add(field);
            }
        }
    ...
    public ModelFormField addUpdateField(ModelFormField modelFormField) {
    	if (!modelFormField.isUseWhenEmpty() || useWhenFields.contains(modelFormField.getName())) {
            ...
            if (existingField != null) {
                ...
                //in this case the field could be in the parent field list
                if (!this.fieldList.contains(existingField)) {
                    this.fieldList.add(modelFormField);
                }
    ...
    public void addAutoFieldsFromService(AutoFieldsService autoFieldsService) {
        ...
        for (ModelParam modelParam: modelService.getInModelParamList()) {
            ...
            if ("userLogin".equals(modelParam.name) || "locale".equals(modelParam.name) || "timeZone".equals(modelParam.name) || "login.username".equals(modelParam.name) || "login.password".equals(modelParam.name) ||
                    (UtilValidate.isNotEmpty(autoFieldsService.excludedList) && autoFieldsService.excludedList.contains(modelParam.name))) {
                continue;
            }
    ...
    public void addAutoFieldsFromEntity(AutoFieldsEntity autoFieldsEntity) {
        ...
        while (modelFieldIter.hasNext()) {
            ...
            if (modelField.getIsAutoCreatedInternal() ||
                    (UtilValidate.isNotEmpty(autoFieldsEntity.excludedList) && autoFieldsEntity.excludedList.contains(modelField.getName()))) {
    ...
    public void renderFormString(Appendable writer, Map<String, Object> context, FormStringRenderer formStringRenderer) throws IOException {
    	...
    	if ("list".equals(this.type) || "multi".equals(this.type)) {
    		boolean incrementPaginator = true;
            Map<String, Object> globalCtx = UtilGenerics.checkMap(context.get("globalContext"));
            if (globalCtx != null) {
                Boolean NO_PAGINATOR = (Boolean) globalCtx.get("NO_PAGINATOR_" + this.name);
                if (UtilValidate.isNotEmpty(NO_PAGINATOR)) {
                    incrementPaginator = false;
                    globalCtx.remove("NO_PAGINATOR_" + this.name);
                }
            }
        ...
        double totalSeconds = (System.currentTimeMillis() - startTime);
        Debug.log("******************** renderFormString(): FINE RENDER FORM ************************* name = " + this.getName() + " in " + totalSeconds + " milliseconds");
    }
    ...
    public void renderSingleFormString(Appendable writer, Map<String, Object> context, FormStringRenderer formStringRenderer, int positions) throws IOException {
        ...
        if (Debug.timingOn()) {
            ...
            Debug.logTiming("Render form " + this.getCurrentFormName(context) + " renderHiddenIgnoredFields in " + (endTime2 - startTime) + " milliseconds", module);
        }
        ...
        while (currentFormField != null) {
            ...
            if (Debug.timingOn()) {
                ...
                Debug.logTiming("Render form " + this.getCurrentFormName(context) + " field " + currentFormField.getName() + " in " + (endTime3 - startTime2) + " milliseconds", module);
            }
        }
        ...
        if (Debug.timingOn()) {
            ...
            Debug.logTiming("Render form " + this.getCurrentFormName(context) + " in " + (endTime - startTime) + " milliseconds", module);
        }
    }
    ...
    public int renderHeaderRow(Appendable writer, Map<String, Object> context, FormStringRenderer formStringRenderer) throws IOException {
        ...
        for (List<ModelFormField> mainFieldList: fieldListsByPosition) {
        	int numOfColumns = 0;
        	List<ModelFormField> submitFormFields = FastList.newInstance();
            List<ModelFormField> modelFormFieldList = FastList.newInstance();

            // render title for each field, except hidden & ignored, etc

            // prepare two lists:
            // one list contains all the fields, included display and hyperlink
            // fields, in the order formatted by sortOrder
            // one list contains the submit field
            // all fields will be rendered as columns
            // For Form with type="list" the hidden fields is in the column with submit
            // For Form with type="multi" there is an another column with the hidden fields
            Iterator<ModelFormField> displayHyperlinkFieldIter = mainFieldList.iterator();
            while (displayHyperlinkFieldIter.hasNext()) {
                ModelFormField modelFormField = displayHyperlinkFieldIter.next();
                ModelFormField.FieldInfo fieldInfo = modelFormField.getFieldInfo();
                ...
                // don't do any header if the use-when condition is true
                // the use.when uses the parameters in the context
                // TODO calculate use-when for every rows
                if (!modelFormField.shouldUse(context)) {
                    continue;
                }

                modelFormFieldList.add(modelFormField);
                numOfColumns++;
            }
            ...
            for (ModelFormField modelFormField: mainFieldList) {
                ...
                submitFormFields.add(modelFormField);
            }
            // add a column for hidden parameters
            ...
            modelFormFieldList = this.renderUserPreference(modelFormFieldList, context );

            Map<String, List<ModelFormField>> fieldRow = UtilMisc.toMap("modelFormFieldList", modelFormFieldList,
                    "submitFields", submitFormFields);
        ...
        for (Map<String, List<ModelFormField>> listsMap: fieldRowsByPosition) {
            List<ModelFormField> modelFormFieldList = listsMap.get("modelFormFieldList");
            List<ModelFormField> submitFormFields = listsMap.get("submitFields");

            int numOfCells = modelFormFieldList.size() +
                        (submitFormFields.size() > 0 && ("list").equals(this.type)? 1: 0);
            if (numOfCells > 0) {
                // render tag <thead> for resize column and fixed header
                formStringRenderer.renderFormatHeaderRowGroupOpen(writer, context, this);
                // render the HeaderRow of FieldGroup if almost one title exists.
                if (fieldGroupList.size() > 1) {
                    formStringRenderer.renderFormatHeaderRowOpen(writer, context, this);
                    // TODO: if field is not contains in FieldGroup, its column has rowspan = 2
                    // IE has problems with rowspan > 1 and position: relative
                    // so its column is empty
                    Iterator<ModelFormField> modelFormFieldListIt = modelFormFieldList.iterator();
                    FieldGroup lastFieldGroup = null;
                    while (modelFormFieldListIt.hasNext()) {
                        ModelFormField modelFormField = modelFormFieldListIt.next();
                        FieldGroup currentFieldGroup = (FieldGroup) fieldGroupMap.get(modelFormField.getFieldName());
                        if (currentFieldGroup == null || currentFieldGroup.getFreeFieldGroup()) {
                            StringBuffer formTitleAreaStyle = new StringBuffer();
                            formTitleAreaStyle.append(this.getFormTitleAreaStyle());
                            formTitleAreaStyle.append(" emptyHeader");
                            if(UtilValidate.isNotEmpty(modelFormField.getTitleAreaStyle()) && modelFormField.getTitleAreaStyle().contains("hidden")) {
                                formTitleAreaStyle.append(" hidden");
                            }
                            this.setFormTitleAreaStyle(formTitleAreaStyle.toString());
                            ...
                            this.setFormTitleAreaStyle("");
                        }
                        else if(lastFieldGroup != currentFieldGroup){
                            formStringRenderer.renderFieldGroupOpen(writer, context, currentFieldGroup);
                            formStringRenderer.renderFieldGroupClose(writer, context, currentFieldGroup);
                            lastFieldGroup = currentFieldGroup;
                        }
                    }
                    formStringRenderer.renderFormatHeaderRowClose(writer, context, this);
                }
                // render the HeaderRow of Fields
                formStringRenderer.renderFormatHeaderRowOpen(writer, context, this);
                Iterator<ModelFormField> modelFormFieldListIt = modelFormFieldList.iterator();
                while (modelFormFieldListIt.hasNext()) {
                    ModelFormField modelFormField = modelFormFieldListIt.next();
                ...
                formStringRenderer.renderFormatHeaderRowClose(writer, context, this);
                formStringRenderer.renderFormatHeaderRowGroupClose(writer, context, this);
            }
        }
    ...
    private List<ModelFormField> renderUserPreference(List<ModelFormField> modelFormFieldList, Map<String, Object> context) {
        // LinkedList add element at the specific position
        // the hidden field must remain at the original position
        // so modelFormFieldLinkedList contains all the element of modelFormFieldList
        // and hiddenBefore is true for all hidden field before the first change
        LinkedList<ModelFormField> modelFormFieldLinkedList = new LinkedList();
        modelFormFieldLinkedList.addAll(modelFormFieldList);
        if (UtilValidate.isNotEmpty(modelFormFieldList)) {
            ModelFormField modelFormField = (ModelFormField) modelFormFieldList.get(0);
            Map<String, Object> paramMap = FastMap.newInstance();
            Map<String, Object> userPrefMap = FastMap.newInstance();
            GenericValue userLogin = (GenericValue) context.get("userLogin");
            paramMap.put("userLogin", userLogin);
            paramMap.put("formName", this.getFormName(context));
            try {
                Map result = this.dispatchContext.getDispatcher().runSync("getWidgetUserPref", paramMap);
                if(!ServiceUtil.isError(result)) {
                    userPrefMap.putAll((Map<String, Object>)result.get("userPrefMap"));
                }
            } catch (GenericServiceException e) {
                Debug.logError(e.getMessage(), module );
            }
            if (UtilValidate.isNotEmpty(userPrefMap)) {
                Set<Entry<String, Object>> values = userPrefMap.entrySet();
                if (values.size() > 0) {
                    for(int countIndex = 0; countIndex < modelFormFieldList.size();countIndex++) {
                        for(Entry<String, Object> entry : values) {
                            String key = entry.getKey();
                            if("formName".equals(key))
                                continue;
                            Map<String, Object> fieldParam = (Map) userPrefMap.get(key);
                            int indexList = this.getIndex(modelFormFieldLinkedList, key, context);
                            int indexMap = indexList;
                            try {
                                indexMap = Integer.parseInt((String)fieldParam.get("position"));

                            } catch (Exception e) {
                                // DO NOTHING
                            }
                            if(indexMap != countIndex)
                                continue;
                            modelFormField = (ModelFormField) modelFormFieldLinkedList.get(indexList);
                            //String groupId = (String)fieldParam.get("groupId");
                            int width = 0;
                            try {
                                width = Integer.parseInt((String)fieldParam.get("width"));
                            } catch (Exception e) {
                                // DO NOTHING
                            }
                            if(width > 0){
                                modelFormField.setWidth(width);
                            }
                            if(indexList > -1 && indexList != indexMap) {
                                if(indexMap >= modelFormFieldLinkedList.size()) {
                                    modelFormFieldLinkedList.addLast(modelFormField);
                                } else {
                                    modelFormFieldLinkedList.add(indexMap, modelFormField);
                                }
                                // if element A is add to the left of its position
                                // the position of the element B is incremented of one
                                if(indexList > indexMap) {
                                    modelFormFieldLinkedList.remove(indexList+1);
                                } else {
                                    modelFormFieldLinkedList.remove(indexList);
                                }
                                break;
                            }
                        }
                ...
                // the field not contained in the list is hidden
                for (int j = 0; j < modelFormFieldList.size(); j++) {
                    modelFormField = (ModelFormField) modelFormFieldList.get(j);
                    String key = this.getFieldName(modelFormField, context);
                    if(!userPrefMap.containsKey(key)) {
                        modelFormField.setTitleAreaStyle("hidden");
                        modelFormField.setWidgetAreaStyle("hidden");
                    }
                }

            }
        }
        return modelFormFieldLinkedList;
    }


    public String getFormName(Map<String, Object> context) {
        return "table_"+this.getContainerId(context);
    }

    public ModelFormField getModelFormField(String name, Map<String, Object> context) {
        ModelFormField res = null;

        if (UtilValidate.isNotEmpty(name) && UtilValidate.isNotEmpty(this.fieldList)) {
            List<ModelFormField> tmpList = this.filterFieldToShow(this.fieldList, context);
            for(ModelFormField field : tmpList) {
    ...
    public String getFieldName(ModelFormField modelFormField, Map<String, Object> context){
        return this.getFormName(context)+this.FIELD_ID_SEPARATOR+modelFormField.getFieldName();
    }


    private int getIndex(LinkedList<ModelFormField> modelFormFieldLinkedList, String key, Map<String, Object> context) {
        for(int i = 0; i< modelFormFieldLinkedList.size(); i++){
            if(key.equalsIgnoreCase(this.getFieldName(modelFormFieldLinkedList.get(i), context))) {
                return i;
            }
        }
        return -1;
...
//            this.incrementPaginatorNumber(context);
    ...
    public void renderItemRows(Appendable writer, Map<String, Object> context, FormStringRenderer formStringRenderer, boolean formPerItem, int numOfColumns) throws IOException {
        ...
        if (lowIndex > listSize) {
            context.put("lowIndex", 0);
            context.put("highIndex", viewSize);
        ...
        if (iter != null) {
            ...
            while ((item = this.safeNext(iter)) != null) {
               ...
               if (itemIndex < lowIndex) {
                ...
                // Check to see if there is a field, same name and same use-when (could come from extended form)
                List<ModelFormField> tempFieldList = filterFieldToShow(this.fieldList, localContext);
                ...
                for (List<ModelFormField> fieldListByPosition: fieldListsByPosition) {
                    ...
                    List<ModelFormField> modelFormFieldList = FastList.newInstance();
                    // Preprocessing:
                    // all the form fields are evaluated and the ones that
                    // appear in the form are put into one lists
                    ...
                    // Rendering:
                    // the fields in the two lists created in the preprocessing phase
                    // are now rendered:
                    if (("list").equals(this.type) && (submitFormFields.size() > 0 || modelFormFieldList.size() > 0)) {
                        this.renderItemRow(writer, localContext, formStringRenderer, formPerItem, hiddenIgnoredFieldList, modelFormFieldList, submitFormFields, currentPosition, numOfColumns);
                    }
                    else if (("multi").equals(this.type) && (submitFormFields.size() > 0 || modelFormFieldList.size() > 0)) {
                        this.renderMultiItemRow(writer, localContext, formStringRenderer, formPerItem, hiddenIgnoredFieldList, modelFormFieldList, submitFormFields, currentPosition, numOfColumns);
                    }
    ...
    // The fields in the modelFormFieldList, usually created in the preprocessing phase
    ...
    public void renderItemRow(Appendable writer, Map<String, Object> localContext, FormStringRenderer formStringRenderer, boolean formPerItem, List<ModelFormField> hiddenIgnoredFieldList, List<ModelFormField> modelFormFieldList, List<ModelFormField> submitFormFields, int position, int numOfColumns) throws IOException {
        boolean firstColumn = true;
        // render row formatting open
        // If there is a submit field the form is open in the row
        formStringRenderer.renderFormatItemRowOpen(writer, localContext, this);
        if(submitFormFields.size() > 0 && formPerItem) {
            formStringRenderer.renderFormOpen(writer, localContext, this);
        }
       
        long startTime2 = System.currentTimeMillis(); 
        
        // render the hidden fields in the first column
        Iterator<ModelFormField> modelFormFieldListIt = modelFormFieldList.iterator();
        while (modelFormFieldListIt.hasNext()) {
            long startTime = System.currentTimeMillis(); 
            ModelFormField modelFormField = (ModelFormField) modelFormFieldListIt.next();
            ...
            if(firstColumn) {
                ...
                if (Debug.timingOn()) {
                    ...
                    Debug.logTiming("Render form " + this.getName() + " renderHiddenIgnoredFields in " + (endTime1 - startTime) + " milliseconds", module);
                }
                firstColumn = false;
            }
            ...
            if (Debug.timingOn()) {
                long endTime2 = System.currentTimeMillis(); 
                Debug.logTiming("Render form " + this.getName() + " field " + modelFormField.getName() + " in " + (endTime2 - startTime) + " milliseconds", module);
            }
            formStringRenderer.renderFormatItemRowCellClose(writer, localContext, this, modelFormField);
            if (Debug.timingOn()) {
                long endTime3 = System.currentTimeMillis(); 
                Debug.logTiming("Render form " + this.getName() + " field " + modelFormField.getName() + " in " + (endTime3 - startTime) + " milliseconds", module);
            }
            
        }
        if (Debug.timingOn()) {
            long endTime3 = System.currentTimeMillis(); 
            Debug.logTiming("Render form " + this + " in " + (endTime3 - startTime2) + " milliseconds", module);
        }
        if(submitFormFields.size() > 0 && formPerItem) {
    ...
    // of the renderItemRows method are rendered: this will create a visual representation
    // of one row (corresponding to one position).
    public void renderMultiItemRow(Appendable writer, Map<String, Object> localContext, FormStringRenderer formStringRenderer, boolean formPerItem, List<ModelFormField> hiddenIgnoredFieldList, List<ModelFormField> modelFormFieldList, List<ModelFormField> submitFormFields, int position, int numOfColumns) throws IOException {
        boolean firstColumn = true;
        // render row formatting open
        formStringRenderer.renderFormatItemRowOpen(writer, localContext, this);
        // render the hidden fields in the first column
    ...
    public String getContainerId(Map<String, Object> context) {
        ...
        if (UtilValidate.isNotEmpty(this.containerId)) {
            return this.containerId.expandString(context);
        } else {
            return this.getName();
        }
    }
    ...
    public String getCurrentContainerId(Map<String, Object> context) {
    	Integer itemIndex = (Integer) context.get("itemIndex");
    	if (itemIndex != null && "list".equals(this.getType())) {
    		return this.getContainerId(context) + this.getItemIndexSeparator() + itemIndex.intValue();
    	}

    	return this.getContainerId(context);
    }
    ...
    public boolean isReadOnly(Map<String, Object> context) {
        return UtilValidate.isNotEmpty(this.readOnlyCondition) && this.readOnlyCondition.eval(context);
    }
    ...
    public void resetBshInterpreter(Map<String, Object> context) {
        // Put null instead of remove to override value in MapStack context.
        context.put("bshInterpreter", null);
    }
    ...
    public void setContainerId(String string) {
        this.containerId = FlexibleStringExpander.getInstance(string);
    }
    ...
    public String getPagerPosition(Map<String, Object> context) {
        return this.pagerPosition.expandString(context);
    }

    public void setPagerPosition(String pagerPosition) {
        this.pagerPosition = FlexibleStringExpander.getInstance(pagerPosition);
    }
    ...
    public String getMultiPaginateIndexField(Map<String, Object> context) {
    	return this.getMultiPaginateIndexField(context, true);
    }

    public String getMultiPaginateIndexField(Map<String, Object> context, boolean withPaginatorNumber) {
    	String field = this.paginateIndexField.expandString(context);
    	if (UtilValidate.isEmpty(field)) {
    		field = DEFAULT_PAG_INDEX_FIELD;
    	}
    	//  append the paginator number
    	if (withPaginatorNumber) {
    ...
    public int getPaginateIndex(Map<String, Object> context) {
        ...
        try {
            ...
            if (value == null) {
                ...
                if (parameters != null) {
                    String paginatorNumber = (String) parameters.get("PAGINATOR_NUMBER");
                    if (UtilValidate.isNotEmpty(paginatorNumber)) {
                        value = parameters.get("VIEW_INDEX" + "_" + paginatorNumber);
                    }

                    if (value == null) {
                        value = parameters.get("VIEW_INDEX" + "_" + WidgetWorker.getPaginatorNumber(context));
                    } else {
                        if (!context.containsKey("REMOVE_PAGINATOR_NUMBER"))
                            context.put("REMOVE_PAGINATOR_NUMBER", Boolean.TRUE);
                    }
    ...
    public String getMultiPaginateSizeField(Map<String, Object> context) {
        return getMultiPaginateSizeField(context, true);
    }

    public String getMultiPaginateSizeField(Map<String, Object> context, boolean withPaginatorNumber) {
    	String field = this.paginateSizeField.expandString(context);
    	if (UtilValidate.isEmpty(field)) {
    		field = DEFAULT_PAG_SIZE_FIELD;
    	}
        ...
        try {
            ...
            if (value == null) {
                ...
                if (parameters != null) {
                    String paginatorNumber = (String) parameters.get("PAGINATOR_NUMBER");
                    if (UtilValidate.isNotEmpty(paginatorNumber)) {
                        value = parameters.get("VIEW_SIZE" + "_" + paginatorNumber);
                    }
    ...
    public void getListLimits(Map<String, Object> context, Object entryList) {
        ...
        if (getPaginate(context)) {
            ...
            if (viewIndex*viewSize >= listSize) {
                viewIndex = (listSize == viewSize ? 0 : listSize/viewSize);
            }
    ...
    public String getSortField(Map<String, Object> context) {
        ...
        try {
            ...
            if (value == null) {
                value = (String)context.get(field + "_table_"+ this.getContainerId(context));
                if (value == null) {
                	Map<String, String> parameters = UtilGenerics.cast(context.get("parameters"));
                    if (parameters != null) {
                        value = (String)parameters.get(field + "_table_"+ this.getContainerId(context));
                        if (value == null) {
    ...
    /** Returns the id of the FieldGroup of modelFormField
     * else return "" */
    public String getFieldGroupId(ModelFormField modelFormField){
        FieldGroupBase fieldGroupBase = this.fieldGroupMap.get(modelFormField.getFieldName());
        return (UtilValidate.isNotEmpty(fieldGroupBase) ? fieldGroupBase.getId() : "");
    }
    ...
    public static class UpdateArea {
        ...
        public UpdateArea(Element updateAreaElement) {
            ...
            this.onlySubmit = UtilXml.checkBoolean(updateAreaElement.getAttribute("only-submit"), false);
        }
        /** String constructor.
    ...
    public static class AutoFieldsService {
        ...
        public AutoFieldsService(Element element) {
            ...
            List<? extends Element> excludedElementList = UtilXml.childElementList(element, "exclude");
            if (UtilValidate.isNotEmpty(excludedElementList)) {
                excludedList = new FastList<String>();
                Iterator<? extends Element> excludedElementIt = excludedElementList.iterator();
                while (excludedElementIt.hasNext()) {
                    Element excludedElement = excludedElementIt.next();
                    excludedList.add(excludedElement.getAttribute("name"));
                }
            }
    ...
    public static class FieldGroup implements FieldGroupBase {
        ...
        public String style;
        public String title;
        public boolean freeFieldGroup = false;
        ...
        public boolean getFreeFieldGroup() {
            return this.freeFieldGroup;
        }

        public void setFreeFieldGroup(boolean freeFieldGroup) {
            this.freeFieldGroup = freeFieldGroup;
        }

        // return the number of fields with this FieldGroup
        public int getFieldNumber() {
            int res = 0;

            Set<Entry<String, FieldGroupBase>> values = this.modelForm.fieldGroupMap.entrySet();
            if (values.size() > 0) {
                for(Entry<String, FieldGroupBase> entry : values) {
                    String fieldName = entry.getKey();
                    String titleAreaStyle = "";
                    if(UtilValidate.isNotEmpty(this.modelForm.fieldMap.get(fieldName))) {
                        titleAreaStyle = this.modelForm.fieldMap.get(fieldName).getTitleAreaStyle();
                    }
                    if (this.equals(entry.getValue()) && (UtilValidate.isEmpty(titleAreaStyle) || (UtilValidate.isNotEmpty(titleAreaStyle) && titleAreaStyle.indexOf("hidden") == -1))) {
                        res++;
                    }
                }
            }
            ...
            if (modelFormField.getFieldInfo() instanceof ModelFormField.FieldInfoWithOptions) {
                for (ModelFormField.OptionSource optionSource: ((ModelFormField.FieldInfoWithOptions)modelFormField.getFieldInfo()).optionSources) {
                    if (optionSource instanceof ModelFormField.EntityOptions) {
                        allEntityNamesUsed.add(((ModelFormField.EntityOptions)optionSource).getEntityName(null));
                    }
                }
            }
        }
