...
public class IterateSectionWidget extends ModelScreenWidget {
    ...
    public ModelScreenWidget getSubWidget(String name, Map<String, Object> context, boolean removeWidget) {
        ...
        if (UtilValidate.isNotEmpty(name)) {
            for (ModelScreenWidget subWidget : sectionList) {
                if (name.equals(subWidget.getExpandedName(context))) {
                    res = subWidget;
                    if (removeWidget)
                        sectionList.remove(subWidget);
                    break;
                } else {
    ...
    public void renderWidgetString(Appendable writer, Map<String, Object> context, ScreenStringRenderer screenStringRenderer) throws GeneralException, IOException {
        ...
        while (iter.hasNext()) {
            ...
            contextMs = contextMs.standAloneChildStack();
    ...
    public <X> void getListLimits(Map<String, Object> context, List<X> items) {
        ...
        if (getPaginate(context)) {
            try {
                String viewIndexString = null;
                String viewSizeString = null;
                String viewIndexParamName = "VIEW_INDEX" + "_";
                String viewSizeParamName = "VIEW_SIZE" + "_";

                Map<String, String> params = UtilGenerics.cast(context.get("parameters"));
                String paginatorNumber = params.get("PAGINATOR_NUMBER");
                if (UtilValidate.isNotEmpty(paginatorNumber)) {
                    viewIndexString = params.get(viewIndexParamName + paginatorNumber);
                    viewSizeString = params.get(viewSizeParamName + paginatorNumber);
                }
                if (UtilValidate.isEmpty(viewIndexString))
                    viewIndexString = params.get(viewIndexParamName + WidgetWorker.getPaginatorNumber(context));
                if (UtilValidate.isEmpty(viewSizeString))
                    viewSizeString = params.get(viewSizeParamName + WidgetWorker.getPaginatorNumber(context));
...
*/
        ...
        if (viewIndex > 0) {
            ...
            linkText.append("PAGINATOR_NUMBER=").append(paginatorNumber).append("&amp;VIEW_SIZE_"+ paginatorNumber + "=").append(viewSize).append("&amp;VIEW_INDEX_" + paginatorNumber + "=").append(viewIndex - 1).append("\"");
        ...
        if (highIndex < listSize) {
            ...
            linkText.append("PAGINATOR_NUMBER=").append("&amp;VIEW_SIZE_" + paginatorNumber + "=").append(viewSize).append("&amp;VIEW_INDEX_" + paginatorNumber + "=").append(viewIndex + 1).append("\"");
