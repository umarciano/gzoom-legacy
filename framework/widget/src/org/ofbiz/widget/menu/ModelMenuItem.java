...
public class ModelMenuItem {
    ...
    protected FlexibleStringExpander alignStyle;
    ...
    public void loadMenuItem(Element fieldElement, ModelMenu modelMenu) {
        ...
        this.setHideIfDisabled(fieldElement.getAttribute("hide-if-disabled"));
        this.disableIfEmpty = fieldElement.getAttribute("disable-if-empty");
        this.align = fieldElement.getAttribute("align");
        if (fieldElement.hasAttribute("align-style"))
            this.setAlignStyle(fieldElement.getAttribute("align-style"));
...
//            permissionChecker = new EntityPermissionChecker(permissionElement);
    ...
    public void setHideIfDisabled(String val) {
        if (UtilValidate.isNotEmpty(val))
            if (val.equalsIgnoreCase("true"))
                hideIfDisabled = Boolean.TRUE;
            else
     ...
     * @param val
     */
    public void setHideIfDisabled(Boolean val) {
        this.hideIfDisabled = val;
    }
    ...
    public Boolean getHideIfDisabled() {
        if (hideIfDisabled != null) {
            ...
            return this.modelMenu.getDefaultHideIfDisabled();
        }
    }
    ...
    public void mergeOverrideModelMenuItem(ModelMenuItem overrideMenuItem) {
        ...
        if (UtilValidate.isNotEmpty(overrideMenuItem.link)) {
            this.link = new Link(this, overrideMenuItem.link);
        }
        if (UtilValidate.isNotEmpty(overrideMenuItem.hideIfDisabled)) {
            this.hideIfDisabled = overrideMenuItem.hideIfDisabled;
        }
        if (UtilValidate.isNotEmpty(overrideMenuItem.actions)) {
            this.actions = overrideMenuItem.actions;
        }
        if (UtilValidate.isNotEmpty(overrideMenuItem.condition)) {
            this.condition = new ModelMenuCondition(this,overrideMenuItem.condition);
        }
    }
    ...
    public void renderMenuItemString(Appendable writer, Map<String, Object> context, MenuStringRenderer menuStringRenderer) throws IOException {
        ...
        if (passed || (!passed && !this.getHideIfDisabled())) {
        	...
        	if (UtilValidate.isNotEmpty(parentPortalPageId)) {
                ...
                localItem.mergeOverrideModelMenuItem(this);
                
                Link localLink = this.getLink();
                
        		for (GenericValue portalPage : portalPages) {
        			if (UtilValidate.isNotEmpty(portalPage.getString("portalPageName"))) {
        				MapStack<String> localContext = MapStack.create(context);
                        localContext.push(portalPage.getAllFields());

                        localItem.setName(portalPage.getString("portalPageId"));

                        String portalPageName = portalPage.getString("portalPageName");
                        if (UtilValidate.isNotEmpty(portalPageName)) {
                            if (portalPageName.indexOf(".") != -1) {
                                String resource = portalPageName.substring(0, portalPageName.indexOf("."));
                                String propertyName = portalPageName.substring(portalPageName.indexOf(".") + 1);

                                try {
                                    portalPageName = UtilProperties.getMessage(resource, propertyName, locale);
                                } catch(Exception e) {
                        ...
                        localItem.setTitle(portalPageName);
                        localItem.link = new Link(localItem, localLink);
                        ...
                        String target = null;
                        if (UtilValidate.isNotEmpty(localItem.getLink())) {
                            Link link = localItem.getLink();
                            if (UtilValidate.isNotEmpty(link.getOnClickUpdateAreas())) {
                                for (UpdateArea area : link.getOnClickUpdateAreas()) {
                                    if (UtilValidate.isEmpty(StringUtil.removeSpaces(area.getAreaTarget(localContext)))) {
                                        area.setAreaTarget("showPortalPage?portalPageId=" + portalPage.getString("portalPageId") + "&parentPortalPageId=" + parentPortalPageId);
                                    }
                                }
                            }
                            if (UtilValidate.isEmpty(link.getTarget(localContext))) {
                                link.setTarget("showPortalPage");
                            } else {
                                target = link.getTarget(localContext);
                                link.setTarget(target);
                            }
                        }

                        menuStringRenderer.renderMenuItem(writer, localContext, localItem);
        			}
        		}
        	} else {
    ...
    public String getTitle(Map<String, Object> context) {
            return UtilValidate.isNotEmpty(title) ? title.expandString(context) : "";
    }
    	...
    	if (UtilValidate.isNotEmpty(parentPortalPageId)) {
    		...
    		try {
    			...
    			if (UtilValidate.isNotEmpty(context.get("userLogin"))) { // check if a user is logged in
    				...
    				/** 
    				 * Prendo tutti i permessi dell utente,
				     * se nn e stato inserito nessun groupId, elimino tutte le portlet con il parent che e statio passato (verra visualizzata solo la prima pagina),
				     * altrimenti viene visualizzate le portlet dove nn ci sono  securityGroupId, o se e presente deve essere prensente anche nello user
				     */
				        
    				List <GenericValue> userLoginSecurityGroupsVi = delegator.findList("UserLoginSecurityGroup", EntityCondition.makeCondition("userLoginId", EntityOperator.EQUALS, userLoginId), null, null, null, false);
				    
    				List <String> listKeySecuriryGroup = FastList.newInstance();
				    if (UtilValidate.isNotEmpty(userLoginSecurityGroupsVi)) {
				    	for (GenericValue userLoginSecurityGroup : userLoginSecurityGroupsVi) {
				       	   listKeySecuriryGroup.add((String)userLoginSecurityGroup.get("groupId"));
				       }        
				    }
				    for (GenericValue portalPage : portalPages) {
				        if(UtilValidate.isEmpty(listKeySecuriryGroup)){
				        	if(portalPage.get("parentPortalPageId").equals(parentPortalPageId)){
				        		portalPages.remove(portalPage);
				        	}
				        }else if (UtilValidate.isNotEmpty(portalPage.get("securityGroupId")) && listKeySecuriryGroup.contains(portalPage.get("securityGroupId"))) {
				                portalPages.remove(portalPage);
				        	}
				    }
    ...
    public void setAlignStyle(String string) {
        this.alignStyle = FlexibleStringExpander.getInstance(string);
    }

    public String getAlignStyle(Map<String, Object> context) {
        if (UtilValidate.isNotEmpty(this.alignStyle)) {
            return this.alignStyle.expandString(context);
        } else if (parentMenuItem != null) {
            return parentMenuItem.getAlignStyle(context);
        } else {
            return this.modelMenu.getDefaultAlignStyle(context);
        }
    }
    ...
    public static class Link {
        ...
        public Link(Element linkElement, ModelMenuItem parentMenuItem) {
            ...
            //Lettura delle liste di area da aggiornare al click!
            List<? extends Element> onEventUpdateAreaElements = UtilXml.childElementList(linkElement, "on-event-update-area");
            for (Element onEventUpdateAreaElement : onEventUpdateAreaElements) {
                addOnEventUpdateArea(new UpdateArea(onEventUpdateAreaElement));
            }
        }
        ...
        public Link(ModelMenuItem parentMenuItem, ModelMenuItem.Link link) {
            this.linkMenuItem = parentMenuItem;

            this.textExdr = link.textExdr;
            this.idExdr = link.idExdr;
            this.styleExdr = link.styleExdr;
            this.targetExdr = link.targetExdr;
            this.targetWindowExdr = link.targetWindowExdr;
            this.prefixExdr = link.prefixExdr;
            this.urlMode = link.urlMode;
            this.fullPath = link.fullPath;
            this.secure = link.secure;
            this.encode = link.encode;
            this.nameExdr = link.nameExdr;
            this.onClickUpdateAreas = link.onClickUpdateAreas;
            this.linkType = link.linkType;
            this.parameterList = FastList.newInstance();
            if (UtilValidate.isNotEmpty(link.parameterList))
            	this.parameterList.addAll(link.parameterList);
            this.requestConfirmation = link.requestConfirmation;
            this.confirmationMsgExdr = link.confirmationMsgExdr;
        }

        public void setLinkMenuItem(ModelMenuItem modelMenuItem) {
        ...
        public void addOnEventUpdateArea(UpdateArea updateArea) {
            ...
            Debug.logVerbose(this.linkMenuItem.getModelMenu().getName() + ":" + this.linkMenuItem.getName() + " adding UpdateArea type " + updateArea.getEventType(), module);
            if ("click".equals(updateArea.getEventType())) {
        ...
        protected void addOnClickUpdateArea(UpdateArea updateArea) {
            if (onClickUpdateAreas == null) {
                onClickUpdateAreas = new LinkedList<UpdateArea>();
            }
            onClickUpdateAreas.add(updateArea);
        }


        //Recupero della lista delle aree da aggiornare
