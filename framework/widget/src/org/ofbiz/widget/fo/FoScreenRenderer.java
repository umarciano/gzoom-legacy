...
public class FoScreenRenderer extends HtmlWidgetRenderer implements ScreenStringRenderer {
    ...
    public static String getFoStyle(String styleName) {
        return getFoStyle(styleName, null);
    }
    public static String getFoStyle(String styleName, Map<String, Object> context) {
    	String value = "";
    	if (UtilValidate.isNotEmpty(context) &&  UtilValidate.isNotEmpty(context.get("foStyleMap"))) {
    		Map<String, String> m = UtilGenerics.checkMap(context.get("foStyleMap"));
    		value = m.get(styleName);
    	} else {
    		value = UtilProperties.getPropertyValue("fo-styles.properties", styleName);
    	}
