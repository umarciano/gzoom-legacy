...
public class LdapAuthenticationServices {
    ...
    public static boolean userLogin(DispatchContext ctx, Map<String, ?> context) {
        ...
        if ("true".equals(UtilProperties.getPropertyValue("security.properties", "security.ldap.ssl.enable"))) {
        ...
        if (userLogin != null) {
            ...
            if (!samePassword) {
                ...
                if (!userLogin.isMutable()) {
                    try {
                        userLogin = delegator.findOne("UserLogin", false, "userLoginId", username);
                    } catch (GenericEntityException e) {
                        Debug.logError(e, "Could not refresh userLogin: " + e.getMessage(), module);
                    }
                }
