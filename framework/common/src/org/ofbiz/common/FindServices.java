...
public class FindServices {
    ...
    public static List<String> emptyOperators;
    public static List<String> rangeOperators;
    ...
    static {
        ...
        emptyOperators = FastList.newInstance();
        emptyOperators.add("empty");
        emptyOperators.add("notEmpty");
        rangeOperators = FastList.newInstance();
        rangeOperators.add("between");
        rangeOperators.add("notBetween");
    }
    ...
    public static List<EntityCondition> createConditionList(Map<String, ? extends Object> parameters, List<ModelField> fieldList, Map<String, Object> queryStringMap, Delegator delegator, Map<String, ?> context) {
        ...
        for (Map.Entry<String, ? extends Object> entry : parameters.entrySet()) {
            ...
            if (fieldName.endsWith("_fld0") || fieldName.endsWith("_fld1")) {
                ...
                if (fieldName.endsWith("_fld1")) {
                	String firstParamFieldName = fieldName.replace("_fld1", "_fld0");
	                if (UtilValidate.isEmpty(operation)) {
	                	key = firstParamFieldName.concat("_op");
	                	if (parameters.containsKey(key)) {
	                		operation = (String) parameters.get(key);
	                		if (UtilValidate.isNotEmpty(operation)) {
	                			if (rangeOperators.contains(operation))
	                				operation += "-2";
	                			else
	                				continue;
	                		}
	                ...
	                if (!parameters.containsKey(fieldName.concat("_ic"))) {
	                	key = firstParamFieldName.concat("_ic");
	                	if (parameters.containsKey(key)) {
	                        ignoreCase = "Y".equals(parameters.get(key));
	                    }
	                }
            ...
            if (ObjectType.isEmpty(fieldValue) && !emptyOperators.contains(operation)) {
                continue;
            }
    ...
    public static EntityCondition createSingleCondition(ModelField modelField, String operation, Object fieldValue, boolean ignoreCase, Delegator delegator, Map<String, ?> context) {
        ...
        if (operation != null) {
            if (operation.equals("contains")) {
                ...
                Object startValue = modelField.getModelEntity().convertFieldValue(modelField, dayStart(timeStampString, 0, (Locale) context.get("locale")), delegator, context);
                ...
                Object endValue = modelField.getModelEntity().convertFieldValue(modelField, dayStart(timeStampString, 1, (Locale) context.get("locale")), delegator, context);
            ...
            } else if (operation.equals("between-2")) {
                fieldOp = EntityOperator.LESS_THAN_EQUAL_TO;
            } else if (operation.equals("notBetween")) {
            	fieldOp = EntityOperator.LESS_THAN;
            } else if (operation.equals("notBetween-2")) {
        ...
        if (fieldOp == EntityOperator.EQUALS && fieldValue instanceof String && ((String) fieldValue).indexOf("| ") > -1) {
            List<EntityCondition> tmpList = FastList.newInstance();
            List<String> fieldValueList = StringUtil.split((String)fieldValue, "| ");
            for (String fieldValueStr : fieldValueList) {
                if (GenericEntity.NULL_FIELD.toString().equals(fieldValueStr)) {
                    tmpList.add(EntityCondition.makeCondition(fieldName, null));
                } else {
                    Object value = modelField.getModelEntity().convertFieldValue(modelField, fieldValueStr, delegator, context);
                    tmpList.add(EntityCondition.makeCondition(fieldName, fieldOp, value));
                }
            }
            cond = EntityCondition.makeCondition(tmpList, EntityOperator.OR);
        } else if (fieldOp == EntityOperator.IN && fieldValue instanceof String) {
        	fieldObject = StringUtil.split((String)fieldValue, ",");
        } else if (fieldOp != EntityOperator.IN || !(fieldValue instanceof Collection<?>)) {
            fieldObject = modelField.getModelEntity().convertFieldValue(modelField, fieldValue, delegator, context);
        }
        if (cond == null) {
            if (GenericEntity.NULL_FIELD.toString().equals(fieldObject)) {
                fieldObject = null;
            }
    ...
    public static List<EntityCondition> createCondition(ModelEntity modelEntity, Map<String, Map<String, Map<String, Object>>> normalizedFields, Map<String, Object> queryStringMap, Map<String, List<Object[]>> origValueMap, Delegator delegator, Map<String, ?> context) {
        ...
        for (ModelField modelField: fields) {
            ...
            /* if opString is "empty", fieldValue is null */
            if (fieldValue == null && !opString.equals("empty")) {
                continue;
            }
            ...
            if (subMap2 == null || (subMap2 != null && subMap2.get("op") == null)) {
                continue;
            }
            fieldValue = subMap2.get("value");
            if (fieldValue == null) {
            ...
            //set up so next part find same operation
            if (!"between".equals(opString) && !"notBetween".equals(opString))
            	opString = (String) subMap2.get("op");
            ignoreCase = "Y".equals(subMap2.get("ic"));
            cond = createSingleCondition(modelField, opString, fieldValue, ignoreCase, delegator, context); 
            tmpList.add(cond);
           ...
           if(opString.equals("notBetween")) {    
        	   int totConditions = tmpList.size();
        	   EntityCondition expr = EntityCondition.makeCondition(tmpList.get(totConditions - 2 ), EntityComparisonOperator.OR, tmpList.get(totConditions -1 ));
           	   tmpList.remove(totConditions - 2);
           	   tmpList.remove(totConditions - 2);
           	   tmpList.add(expr);
           }
        }
    ...
    public static Map prepareFind(DispatchContext dctx, Map<String, ?> context) {
        ...
        if (tmpList.size() > 0 || "Y".equals(noConditionFind)) {
            if ("Y".equals(filterByDate)) {    
                ...
                if (EntityUtil.isFilterByDateCompatible(modelEntity, thruDateName)) {
    ...
    private static String dayStart(String timeStampString, int daysLater, Locale locale) {
        ...
        try {
            ts = UtilDateTime.toTimestamp(timeStampString, locale);
        } catch (Exception e) {
            return retValue;
        }
    ...
    public static Map<String, Object> buildReducedQueryString(Map<String, ?> inputFields, String entityName, Delegator delegator, Locale locale) {
        // Strip the "_suffix" off of the parameter name and
        // build a three-level map of values keyed by fieldRoot name,
        //    fld0 or fld1,  and, then, "op" or "value"
        // ie. id
        ...
        // Those extra fields will be ignored in the second half of this method.
        ModelEntity modelEntity = delegator.getModelEntity(entityName);
        Map<String, Object> normalizedFields = new FastMap<String, Object>();
        //StringBuffer queryStringBuf = new StringBuffer();
        for (String fieldNameRaw: inputFields.keySet()) { // The name as it appeas in the HTML form
            String fieldNameRoot = null; // The entity field name. Everything to the left of the first "_" if
                                                                 //  it exists, or the whole word, if not.
            ...
            if (modelEntity.isField(fieldNameRoot)) {
                ...
                if (fieldNameRaw.indexOf("_op") > -1) {
                    String fieldName = fieldNameRaw.substring(0, fieldNameRaw.indexOf("_op"));
                    String fieldNameRawLocale = fieldName + "_loc";
                    String fieldValueLocale = "";
                    if("equals".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "equals", locale);
                    } else if("notEqual".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "not_equal", locale);
                    } else if("empty".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "is_empty", locale);
                    } else if("notEmpty".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "is_not_empty", locale);
                    } else if("between".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "between", locale);
                    } else if("notBetween".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "not_between", locale);
                    } else if("like".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "begins_with", locale);
                    } else if("lessThan".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "less_than", locale);
                    } else if("greaterThan".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "greater_than", locale);
                    } else if("contains".equals(fieldValue)) {
                        fieldValueLocale += UtilProperties.getMessage("conditional", "contains", locale);
                    } else {
                        fieldValueLocale = fieldValue.toString();
                    }
                    normalizedFields.put(fieldNameRawLocale, fieldValueLocale);
                }
            }
        }
        return normalizedFields;
