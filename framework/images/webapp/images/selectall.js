...
function createEventPanel(f) {
    var eventPanel = new Element('div', {id : 'eventpanel'});
    $(document.body).insert({Top:eventPanel});
    Event.observe(eventPanel, 'dom:ajaxload', f);
}
...
function ajaxAutoCompleteTextFind(descriptionElement, target, options) {
    var update = $(descriptionElement).identify() + "_autoCompleterOptions";
    $(descriptionElement).insert({after: '<div class="autocomplete"' + 'id=' + update + '></div>'});
    new Ajax.Autocompleter($(descriptionElement), update, target, {parameters: options.parameters, autoSelect: options.autoSelect, frequency: options.frequency, minChars: options.minChars, choices: options.choices, partialSearch: options.partialSearch, partialChars: options.partialChars, ignoreCase: options.ignoreCase, fullSearch: options.fullSearch, afterUpdateElement: options.afterUpdateElement, onShow : options.onShow});
}
...
//Load dimamically js file described in urls (more than one js file separate by space character)
//synchronousOptions : is exist load the js file synchronous. It contains:
//	- symbol: check the existens of this symbol in he main NameSpace. If exists, stop waiting
//  - onSuccess : callback function at the end of loading
function dhtmlLoadScript(urls, synchronousOptions)
{
    if (urls) {
        synchronousOptions = synchronousOptions || {};
        var onComplete = synchronousOptions.onComplete;

        var hashMap = $H(synchronousOptions);
        hashMap.unset('onComplete');
        synchronousOptions = hashMap.toObject();

        var urlArray = $w(urls);
        urlArray.each(function(url, index) {
            if (!synchronousOptions.conditionForLoad || (Object.isString(synchronousOptions.conditionForLoad) && evalBool('return ' + synchronousOptions.conditionForLoad))) {
                if (index == urlArray.size()-1 && Object.isFunction(onComplete)) {
                    synchronousOptions['onComplete'] = onComplete;
                    onComplete = null;
                }
                new Ajax.Request(url, synchronousOptions);
            }
        });

        if (onComplete != null && Object.isFunction(onComplete)) {
            onComplete();
        }
    }
}
...
/**
 * Parse action url (ex: doSome?par1=xx) into complete ofbiz/current application url -> http://server:port/currentApplication/control/doSome?par1=xx
 * (This function is based on Flog.UriParser lib into uriparser.js
 * */
function getOfbizUrl(action) {

    if(typeof(Flog) == "undefined")
        throw "Url parsing requires UriParser to be loaded.";

    //get current uri
    var currentUrl = window.location;
    var parser = new Flog.UriParser(currentUrl);

    //looking useful url part
    var currentPath = parser.pathname;
    var pos = currentPath.toLowerCase().lastIndexOf("control/");
    if (pos==-1) {
        return currentPath + action;
    }
    pos += 8;
    var newPath = currentPath.substr(0, pos);

    return newPath + action;
}