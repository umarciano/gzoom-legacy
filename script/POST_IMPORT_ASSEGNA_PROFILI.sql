...
--   A) ORGPERF_REFERENTE  -> responsabili (ORG_RESPONSIBLE) delle UOC-referente (indicatori WEM_IND_IN_CHARGE)
--   B) ORGPERF_DIR_UO     -> responsabili (ORG_RESPONSIBLE) delle UO con scheda CTX_BS (per validare la propria scheda)
...
SELECT DISTINCT ul.user_login_id, 'ORGPERF_REFERENTE', TIMESTAMP '2026-01-01 00:00:00',
       now(), now(), now(), now()
...
WHERE gar.role_type_id = 'WEM_IND_IN_CHARGE'
  ...
  AND NOT EXISTS (SELECT 1 FROM user_login_security_group x
                  WHERE x.user_login_id = ul.user_login_id AND x.group_id = 'ORGPERF_REFERENTE' AND x.thru_date IS NULL);

-- ---------- B) DIRETTORI UO (85) ----------
INSERT INTO user_login_security_group (user_login_id, group_id, from_date,
       last_updated_stamp, last_updated_tx_stamp, created_stamp, created_tx_stamp)
SELECT DISTINCT ul.user_login_id, 'ORGPERF_DIR_UO', TIMESTAMP '2026-01-01 00:00:00',
       now(), now(), now(), now()
FROM work_effort we
...
WHERE we.work_effort_type_id = 'CTX_BS' AND we.org_unit_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM user_login_security_group x
                  WHERE x.user_login_id = ul.user_login_id AND x.group_id = 'ORGPERF_DIR_UO' AND x.thru_date IS NULL);

-- ---------- Verifica ----------
SELECT group_id, count(*) AS membri
FROM user_login_security_group
WHERE group_id IN ('ORGPERF_REFERENTE','ORGPERF_DIR_UO') AND thru_date IS NULL
