...
public class EntityFieldValue extends EntityConditionValue implements Reusable {
    ...
    @Override
    public void addSqlValue(StringBuilder sql, Map<String, String> tableAliases, ModelEntity modelEntity, List<EntityConditionParam> entityConditionParams, boolean includeTableNamePrefix, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        if (this.modelViewEntity != null) {
            ...
            if (UtilValidate.isNotEmpty(entityAlias)) {
                ...
                String colName = modelField.getColName();
                if (memberModelEntity instanceof ModelViewEntity) {
                    colName = colName.replaceAll("\\.", "_").replaceAll("\\(", "_").replaceAll("\\)", "_");
                }
