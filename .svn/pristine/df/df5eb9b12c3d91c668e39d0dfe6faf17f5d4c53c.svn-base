...
//CREO EVENDO REMINDER_ASS_OBI_2
...
//CREO EVENTO REMINDER_ASS_OBI_2 ASSOC
...
//CREO EVENTO SOLLECITO OBIETTIVO
conditionReminder = EntityCondition.makeCondition(EntityCondition.makeCondition("jobName", "Sollecito Stampa Obiettivi Individuali Duplicato 2"),
    EntityCondition.makeCondition("statusId",  "SERVICE_PENDING")
);
...
if (UtilValidate.isEmpty(reminderScheduled) || reminderScheduled.size() <= 0) {
    ...
    jobSandBoxReminder.put("jobId", "REMINDER_ASS_OBI_2");
    jobSandBoxReminder.put("jobName", "Sollecito Stampa Obiettivi Individuali Duplicato");
    jobSandBoxReminder.put("runTime", new Timestamp(UtilDateTime.toDate(1, 1, 2000, 0, 0, 0).getTime()));
    jobSandBoxReminder.put("serviceName", "reminderAssObi2Scheduled");
...
//CREO EVENTO SOLLECITO OBIETTIVO
conditionReminder = EntityCondition.makeCondition(EntityCondition.makeCondition("jobName", "Sollecito Stampa Obiettivi Individuali Duplicato 1"),
  EntityCondition.makeCondition("statusId",  "SERVICE_PENDING")
);
reminderScheduled = delegator.findList("JobSandbox", conditionReminder, null, null, null, false);
Debug.log("## reminderScheduled:"+reminderScheduled);
if (UtilValidate.isEmpty(reminderScheduled) || reminderScheduled.size() <= 0) {
  //inserisco
  jobSandBoxReminder = delegator.makeValue("JobSandbox");
  jobSandBoxReminder.put("jobId", "REMINDER_ASS_OBI_1");
  jobSandBoxReminder.put("jobName", "Sollecito Stampa Obiettivi Individuali Duplicato 1");
  jobSandBoxReminder.put("runTime", new Timestamp(UtilDateTime.toDate(1, 1, 2000, 0, 0, 0).getTime()));
  jobSandBoxReminder.put("serviceName", "reminderAssObi1Scheduled");
  jobSandBoxReminder.put("poolId", "pool");
  jobSandBoxReminder.put("runAsUser", "admin");
  jobSandBoxReminder.put("tempExprId", "REMINDER");
  jobSandBoxReminder.put("maxRecurrenceCount", -1L);
