SELECT A.WORK_EFFORT_ID,
       ...
       A.ETCH AS WE_ETCH,
       U.ULVR_USER_LOGIN_ID AS UV_USER_LOGIN_ID,
       ...
       D.GL_FISCAL_TYPE_ID,
       D.NEXT_STATUS_ID,
       H.WORK_EFFORT_ID AS WORK_EFFORT_ID_CHILD,
       H.WORK_EFFORT_TYPE_ID,
       L.ETCH AS WE_TYPE_ETCH,
       L.ETCH_LANG AS WE_TYPE_ETCH_LANG,
       Count(H.WORK_EFFORT_TYPE_ID) AS WE_TOTAL,
       I.PARTY_ID AS PARTY_ID_IN_CHARGE,
       N.PARTY_NAME AS PARTY_NAME_IN_CHARGE,
       I.ROLE_TYPE_ID AS ROLE_TYPE_ID_IN_CHARGE,
       D.MANAGEMENT_ROLE_TYPE_ID,
       D.MANAG_WE_STATUS_ENUM_ID,
       E.PARTY_ID_TO,
       F.PARTY_ID,
       Min(S.SEQUENCE_ID) AS MIN_SEQUENCE_ID
FROM   <@table "WORK_EFFORT"/> A
       INNER JOIN (SELECT UL.PARTY_ID        AS UL_PARTY_ID,
                   ...
                   FROM   <@table "USER_LOGIN_VALID_PARTY_ROLE"/> ULVR
                          INNER JOIN <@table "USER_LOGIN"/> UL
                                  ON ULVR.USER_LOGIN_ID = UL.USER_LOGIN_ID) U
               ON A.ORGANIZATION_ID = U.ULVR_PARTY_ID
                  AND ( U.ULVR_ROLE_TYPE_ID = 'INTERNAL_ORGANIZATIO' )
       LEFT JOIN <@table "WORK_EFFORT_ASSOC"/> G
              ON A.WORK_EFFORT_ID = G.WORK_EFFORT_ID_FROM
                 AND ( G.WORK_EFFORT_ASSOC_TYPE_ID = 'ROOT' )
       LEFT JOIN <@table "WORK_EFFORT"/> H
              ON G.WORK_EFFORT_ID_TO = H.WORK_EFFORT_ID
       INNER JOIN <@table "WORK_EFFORT_TYPE"/> L
               ON H.WORK_EFFORT_TYPE_ID = L.WORK_EFFORT_TYPE_ID
       LEFT JOIN <@table "WORK_EFFORT_PARTY_ASSIGNMENT"/> I
              ON A.WORK_EFFORT_ID = I.WORK_EFFORT_ID
                 AND ( I.ROLE_TYPE_ID LIKE 'WEM%IN_CHARGE' )
       INNER JOIN <@table "WORK_EFFORT_TYPE"/> B
               ON A.WORK_EFFORT_TYPE_ID = B.WORK_EFFORT_TYPE_ID
       INNER JOIN <@table "STATUS_ITEM"/> C
               ON A.CURRENT_STATUS_ID = C.STATUS_ID
       LEFT OUTER JOIN <@table "WORK_EFFORT_TYPE_STATUS"/> D
                    ON A.WORK_EFFORT_TYPE_ID = D.WORK_EFFORT_TYPE_ROOT_ID
                       AND A.CURRENT_STATUS_ID = D.CURRENT_STATUS_ID
       LEFT OUTER JOIN <@table "STATUS_VALID_CHANGE"/> V
                    ON A.CURRENT_STATUS_ID = V.STATUS_ID
       LEFT OUTER JOIN <@table "STATUS_ITEM"/> S
                    ON V.STATUS_ID_TO = S.STATUS_ID
       LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> E
                    ON A.ORG_UNIT_ROLE_TYPE_ID = E.ROLE_TYPE_ID_FROM
                       AND A.ORG_UNIT_ID = E.PARTY_ID_FROM
                       AND (( E.PARTY_RELATIONSHIP_TYPE_ID IN (
                              'ORG_RESPONSIBLE', 'ORG_DELEGATE' )
                              AND ( E.FROM_DATE <= A.ESTIMATED_COMPLETION_DATE
                                    AND ( E.THRU_DATE IS NULL
                                           OR E.THRU_DATE >=
                                              A.ESTIMATED_COMPLETION_DATE ) )
                              AND E.PARTY_ID_TO = U.UL_PARTY_ID ))
       LEFT OUTER JOIN <@table "WORK_EFFORT_PARTY_ASSIGNMENT"/> F
                    ON A.WORK_EFFORT_ID = F.WORK_EFFORT_ID
                       AND A.ESTIMATED_COMPLETION_DATE = F.THRU_DATE
                       AND ( F.PARTY_ID = U.UL_PARTY_ID )
       LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> Z
       ...
       LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> Y
      ...
      LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> Z2
      ...
      LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> Y2
       ...
       LEFT OUTER JOIN <@table "PARTY"/> M
                    ON A.ORG_UNIT_ID = M.PARTY_ID
       LEFT OUTER JOIN <@table "PARTY"/> N
                    ON I.PARTY_ID = N.PARTY_ID
WHERE  ( 1 = 1         
...
<#else>
    <#if weContextId?if_exists == 'CTX_EP' || weContextId?if_exists == 'CTX_DI'>
        ORDER BY M.PARTY_NAME ASC, N.PARTY_NAME ASC, A.ESTIMATED_START_DATE ASC, A.CURRENT_STATUS_ID ASC
    <#else>
