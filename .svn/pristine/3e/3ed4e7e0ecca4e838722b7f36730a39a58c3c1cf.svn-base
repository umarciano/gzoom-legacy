...
public class WidgetWorker {
	...
	public static void makeHyperlinkString(Appendable writer, String linkStyle, String targetType, String target, Map<String, String> parameterMap,
		...
		if (UtilValidate.isNotEmpty(description) || UtilValidate.isNotEmpty(request.getAttribute("image"))) {
			...
			String ajaxCall = UtilValidate.isNotEmpty(ajaxParameters) ? "ajaxUpdateAreas('" + ajaxParameters + "'); return false;" : null;
			if (UtilValidate.isNotEmpty(confirmation)){
				if (UtilValidate.isEmpty(ajaxCall)) {
					writer.append(" onclick=\"return confirm('");
					writer.append(confirmation);
					writer.append("')\"");
				} else {
					writer.append(" onclick=\"if confirm('");
					writer.append(confirmation);
					writer.append("') {");
					writer.append(ajaxCall);
					writer.append("} else return false;\"");
				}
			} else {
	...
	public static class Parameter {
	    ...
	    public Parameter(Parameter parameter) {
	    	...
	    	if (UtilValidate.isNotEmpty(parameter.value)) {
	    		this.value = FlexibleStringExpander.getInstance(parameter.value.getOriginal());
	    	} else {
	    		this.value = FlexibleStringExpander.getInstance("");
	    	}
	    	if (UtilValidate.isNotEmpty(parameter.fromField)) {
	    		this.fromField = FlexibleMapAccessor.getInstance(parameter.fromField.getOriginalName());
	    	} else {
	    		this.fromField = FlexibleMapAccessor.getInstance("");
	    	}
	    }
