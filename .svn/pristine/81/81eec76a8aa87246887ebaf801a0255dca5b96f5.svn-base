...
public abstract class EntityComparisonOperator<L, R> extends EntityOperator<L, R, Boolean> {
    ...
    @Override
    public void addSqlValue(StringBuilder sql, ModelEntity entity, List<EntityConditionParam> entityConditionParams, boolean compat, L lhs, R rhs, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        ...
        if (lhs instanceof EntityConditionValue) {
            EntityConditionValue ecv = (EntityConditionValue) lhs;
            if ((this.idInt == EntityOperator.ID_NOT_EQUAL || this.idInt == EntityOperator.ID_NOT_EQUAL_FIELD) && (datasourceInfo != null && datasourceInfo.useNotEqualsOrNullClause)) {
            	sql.append("(");
            	
            	ecv.addSqlValue(extraCondition, entity, entityConditionParams, false, datasourceInfo, modelFieldTypeReader);
            }
            
            ecv.addSqlValue(sql, entity, entityConditionParams, false, datasourceInfo, modelFieldTypeReader);
            field = ecv.getModelField(entity);
        } else if (compat && lhs instanceof String) {
            field = getField(entity, (String) lhs);
            if (field == null) {
                sql.append(lhs);
                extraCondition.append(lhs);
            } else {
            ...
            addValue(extraCondition, null, lhs, entityConditionParams);
            field = null;
        }

        makeRHSWhereString(entity, entityConditionParams, sql, field, rhs, datasourceInfo, modelFieldTypeReader);
        
        if (UtilValidate.isNotEmpty(extraCondition) && (this.idInt == EntityOperator.ID_NOT_EQUAL || this.idInt == EntityOperator.ID_NOT_EQUAL_FIELD) && (datasourceInfo != null && datasourceInfo.useNotEqualsOrNullClause)) {
        	sql.append(" OR ");
        	
        	sql.append(extraCondition.toString()).append(" IS NULL)");
        }
    }

    protected void makeRHSWhereString(ModelEntity entity, List<EntityConditionParam> entityConditionParams, StringBuilder sql, ModelField field, R rhs, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        sql.append(' ').append(getCode()).append(' ');
        makeRHSWhereStringValue(entity, entityConditionParams, sql, field, rhs, datasourceInfo, modelFieldTypeReader);
    }

    protected void makeRHSWhereStringValue(ModelEntity entity, List<EntityConditionParam> entityConditionParams, StringBuilder sql, ModelField field, R rhs, DatasourceInfo datasourceInfo, ModelFieldTypeReader modelFieldTypeReader) {
        if (rhs instanceof EntityConditionValue) {
            EntityConditionValue ecv = (EntityConditionValue) rhs;
            ecv.addSqlValue(sql, entity, entityConditionParams, false, datasourceInfo, modelFieldTypeReader);
        } else {
    ...
    public abstract Object getRHS(Object rhs);
