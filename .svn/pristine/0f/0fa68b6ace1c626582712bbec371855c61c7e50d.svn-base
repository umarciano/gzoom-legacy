IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_TEMPLATE_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_TEMPLATE_VIEW
    END;
 ...
 CREATE
	VIEW PARTY_TEMPLATE_VIEW AS SELECT
		PH.PARTY_ID,
		CASE
			WHEN PH.FROM_DATE IS NULL THEN '20000101'
			ELSE PH.FROM_DATE
		END AS FROM_DATE,
		PH.THRU_DATE,
		EP.TEMPLATE_ID,
...
UNION SELECT
		PER.PARTY_ID,
		CASE
			WHEN PER.EMPL_POSITION_TYPE_DATE IS NULL THEN '20000101'
			ELSE PER.EMPL_POSITION_TYPE_DATE
		END AS FROM_DATE,
		'20991231' AS THRU_DATE,
		EP.TEMPLATE_ID,
		EP.EMPL_POSITION_TYPE_ID,
		WE.WORK_EFFORT_TYPE_ID AS TEMPLATE_TYPE_ID,
		WT.PARENT_TYPE_ID
...
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.ORG_ALLOCATION_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW ORG_ALLOCATION_VIEW
    END;

IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.FROM_DATE_ALLOCATION_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW FROM_DATE_ALLOCATION_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.THRU_DATE_ALLOCATION_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW THRU_DATE_ALLOCATION_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.FROM_DATE_ALLOCATION_VIEW_TOT')
                    AND type = 'V') 
    BEGIN
        DROP VIEW FROM_DATE_ALLOCATION_VIEW_TOT
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.THRU_DATE_ALLOCATION_VIEW_TOT')
                    AND type = 'V') 
    BEGIN
        DROP VIEW THRU_DATE_ALLOCATION_VIEW_TOT
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.FROM_THRU_DATE_ALLOCATION_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW FROM_THRU_DATE_ALLOCATION_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.ALLOCATION_TEMPLATE_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW ALLOCATION_TEMPLATE_VIEW
    END;
 ...
 CREATE
	VIEW ORG_ALLOCATION_VIEW AS SELECT
		...
		PREL.FROM_DATE,
		CASE
			WHEN PREL.THRU_DATE IS NULL THEN '20991231'
			ELSE PREL.THRU_DATE
		END AS THRU_DATE,
...
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.ORG_EMPLOYMENT_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW ORG_EMPLOYMENT_VIEW
    END;

IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.FROM_DATE_EMPLOYMENT_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW FROM_DATE_EMPLOYMENT_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.THRU_DATE_EMPLOYMENT_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW THRU_DATE_EMPLOYMENT_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.FROM_DATE_EMPLOYMENT_VIEW_TOT')
                    AND type = 'V') 
    BEGIN
        DROP VIEW FROM_DATE_EMPLOYMENT_VIEW_TOT
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.THRU_DATE_EMPLOYMENT_VIEW_TOT')
                    AND type = 'V') 
    BEGIN
        DROP VIEW THRU_DATE_EMPLOYMENT_VIEW_TOT
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.FROM_THRU_DATE_EMPLOYMENT_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW FROM_THRU_DATE_EMPLOYMENT_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.EMPLOYMENT_TEMPLATE_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW EMPLOYMENT_TEMPLATE_VIEW
    END;
