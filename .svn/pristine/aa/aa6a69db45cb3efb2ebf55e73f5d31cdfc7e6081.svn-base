    ...
    load: function(workEffortId) {
        ...
        <#list portletAssocList as portletAssocAssoc>
            ...
            <#if portalPagePortletIdentifications?has_content>
                ...
                ajaxUpdateArea('portlet-container_${portalPageId}_${portalPortletId}_${portletSeqId}', '<@ofbizUrl>${parameters._LAST_VIEW_NAME_}</@ofbizUrl>','workEffortId='+workEffortId+'&portalPageId=${portalPageId?if_exists}&portalPortletId=${portalPortletId?if_exists}&portletSeqId=${portletSeqId?if_exists}&saveView=N');
            </#if>
        </#list>
    }
}
