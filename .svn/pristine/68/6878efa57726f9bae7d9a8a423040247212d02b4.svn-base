IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_ROLLUP_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_ROLLUP_VIEW
    END;

IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_ROLLUP4_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_ROLLUP4_VIEW
    END;

IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_ROLLUP3_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_ROLLUP3_VIEW
    END;

IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_ROLLUP2_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_ROLLUP2_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_ROLLUP1_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_ROLLUP1_VIEW
    END;
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.PARTY_ROLLUP0_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW PARTY_ROLLUP0_VIEW
    END;
