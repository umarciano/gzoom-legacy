 SELECT 
  ...
  INNER JOIN <@table "GL_ACCOUNT_ROLE"/> AR ON AR.GL_ACCOUNT_ID = AC.GL_ACCOUNT_ID
  INNER JOIN <@table "ROLE_TYPE"/> RT ON (RT.ROLE_TYPE_ID = AR.ROLE_TYPE_ID AND RT.PARENT_TYPE_ID <> 'ORGANIZATION_UNIT')
  INNER JOIN <@table "ACCTG_TRANS_ENTRY"/> TE ON (TE.GL_ACCOUNT_ID = IC.GL_ACCOUNT_ID_REF
         AND TE.PARTY_ID = AR.PARTY_ID AND TE.ROLE_TYPE_ID = AR.ROLE_TYPE_ID AND TE.AMOUNT IS NOT NULL)
  ...
  GROUP BY AR.PARTY_ID, AR.ROLE_TYPE_ID, WE.WORK_EFFORT_ID, WM.WORK_EFFORT_MEASURE_ID
