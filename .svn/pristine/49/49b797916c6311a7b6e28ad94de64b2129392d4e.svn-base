...
public abstract class GenericAsyncEngine extends AbstractEngine {
    ...
    private static final String PROP_MILLS_AFTER_SCHED = "AsyncEngine.millisecondsAfterScheduled";
    ...
    public void runAsync(String localName, ModelService modelService, Map<String, Object> context, GenericRequester requester, boolean persist) throws GenericServiceException {
        ...
        if (persist) {
            ...
            try {
                ...
                Map<String, Object> jFields = UtilMisc.toMap("jobId", jobId, "jobName", (Object)jobName);
                
                jFields.put("runTime", getMillisAfterScheduled());
                ...
                jobV.create();
                if (Debug.timingOn()) { 
                    Debug.log("Create JobSandbox " + jobV);
                }
    ...
    public static Timestamp getMillisAfterScheduled() {
        try {
            String prop = UtilProperties.getPropertyValue("BaseConfig", PROP_MILLS_AFTER_SCHED);
            
            if (UtilValidate.isNotEmpty(prop)) {
                long millisAfterScheduled = Integer.valueOf(prop);
                return  new java.sql.Timestamp(System.currentTimeMillis() - (millisAfterScheduled));
            }
