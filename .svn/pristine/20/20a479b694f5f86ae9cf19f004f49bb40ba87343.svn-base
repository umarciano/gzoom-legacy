...
if(UtilValidate.isNotEmpty(securityGroupList)) {
	securityGroupList.each{ securityGroupItem ->
		...
		securityGroupPermissionConditions.add(EntityCondition.makeCondition("permissionId", "EMPLPERFMGR_VIEW"));
		...
		if(UtilValidate.isNotEmpty(securityGroupPermissionList)) {	
			...
			securityGroupContentConditions.add(EntityCondition.makeCondition("contentId", "GP_MENU_00484"));
		    ...
		    if (UtilValidate.isEmpty(securityGroupContentList)) {
		    	...
		    	securityGroupContent.put("contentId", "GP_MENU_00484");
