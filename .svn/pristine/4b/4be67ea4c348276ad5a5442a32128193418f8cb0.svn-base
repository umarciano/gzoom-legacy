...
if(UtilValidate.isNotEmpty(userLoginSecurityGroupList)) {
	userLoginSecurityGroupList.each{ userLoginSecurityGroupItem ->
		def noPortalDirConditions = [];
		noPortalDirConditions.add(EntityCondition.makeCondition("userLoginId", userLoginSecurityGroupItem.userLoginId));
		noPortalDirConditions.add(EntityCondition.makeCondition("groupId", "NOPORTAL_DIR"));
		def noPortalDirList = delegator.findList("UserLoginSecurityGroup", EntityCondition.makeCondition(noPortalDirConditions), null, null, null, false);
	
	    if (UtilValidate.isEmpty(noPortalDirList)) {
	    	...
	    	userLoginSecurityGroup.put("groupId", "NOPORTAL_DIR");
...
if(UtilValidate.isNotEmpty(securityGroupList)) {
	securityGroupList.each{ securityGroupItem ->
	    ...
	    if (UtilValidate.isNotEmpty(userLoginSecurityGroups)) {
			...
			securityGroupContentConditions.add(EntityCondition.makeCondition("contentId", "NOPORTAL_DIR"));
		    ...
		    if (UtilValidate.isEmpty(securityGroupContentList)) {
		    	...
		    	securityGroupContent.put("contentId", "NOPORTAL_DIR");
