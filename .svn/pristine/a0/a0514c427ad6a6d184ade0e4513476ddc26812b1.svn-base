...
public class JobInvoker implements Runnable {
    ...
    private int count = 0;
    private int wait = 0;
    private String jobInvokerAppModule = null;
    ...
    public JobInvoker(JobPoller jp, int wait) {
        ...
        this.jobInvokerAppModule = UtilProperties.getPropertyValue("general.properties", "logMDC.jobInvokerAppModule");
    ...
    public synchronized void run() {
        while (run) {
            Debug.putMDC("appModule", jobInvokerAppModule);
            Job job = jp.next();
