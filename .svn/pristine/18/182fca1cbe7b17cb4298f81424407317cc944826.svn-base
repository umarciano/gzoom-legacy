...
public abstract class ModelScreenAction implements Serializable {
    ...
    public static List<ModelScreenAction> readSubActions(ModelScreen modelScreen, Element parentElement) {
        ...
        for (Element actionElement: actionElementList) {
            ...
            /* Maps spa - added log tag */     
            } else if ("log".equals(actionElement.getNodeName())) {
            	actions.add(new Log(modelScreen, actionElement));
            /* Maps end */	
            } else {
    ...
    public static class Script extends ModelScreenAction {
        ...
        protected FlexibleStringExpander location;
        ...
        public Script(ModelScreen modelScreen, Element scriptElement) {
            ...
            this.location = FlexibleStringExpander.getInstance(WidgetWorker.getScriptLocation(scriptLocation));
        ...
        public void runAction(Map<String, Object> context) throws GeneralException {
            ...
            String locationExdr = this.location.expandString(context);

            if (locationExdr.endsWith(".bsh")) {
                try {
                    BshUtil.runBshAtLocation(locationExdr, context);
                } catch (GeneralException e) {
                    throw new GeneralException("Error running BSH script at location [" + locationExdr + "]", e);
                }
            } else if (locationExdr.endsWith(".groovy")) {
            	try {
            		groovy.lang.Script script = InvokerHelper.createScript(GroovyUtil.getScriptClassFromLocation(locationExdr), GroovyUtil.getBinding(context));
            		...
            		throw new GeneralException("Error running Groovy script at location [" + locationExdr + "]", e);
            	}
            } else if (locationExdr.endsWith(".xml")) {
                Map<String, Object> localContext = FastMap.newInstance();
                localContext.putAll(context);
                DispatchContext ctx = this.modelScreen.getDispatcher(context).getDispatchContext();
                MethodContext methodContext = new MethodContext(ctx, localContext, null);
                try {
                	SimpleMethod.runSimpleMethod(locationExdr, method, methodContext);
                    context.putAll(methodContext.getResults());
                } catch (MiniLangException e) {
                    throw new GeneralException("Error running simple method at location [" + locationExdr + "]", e);
                }
            } else {
                throw new GeneralException("For screen script actions the script type is not yet supported for location: [" + locationExdr + "]");
            }
            if (Debug.timingOn()) {
                long endTime = System.currentTimeMillis(); 
                Debug.log("Run action script in " + (endTime - startTime) + " milliseconds. script = " + locationExdr);
            }
        }
    }
    ...
    public static class Service extends ModelScreenAction {
        ...
        public void runAction(Map<String, Object> context) {
            ...
            if (Debug.timingOn()) {
                long endTime = System.currentTimeMillis(); 
                Debug.log("Run action service in " + (endTime - startTime) + " milliseconds. serviceName = " + serviceNameExpanded);
            }
        }
    }
    ...
    public static class EntityAnd extends ModelScreenAction {
        ...
        public void runAction(Map<String, Object> context) {
            ...
            if (Debug.timingOn()) {
                long endTime = System.currentTimeMillis(); 
                Debug.log("Run action entityAnd in " + (endTime - startTime) + " milliseconds");
            }
        }
    }
    ...
    public static class EntityCondition extends ModelScreenAction {
        ...
        public void runAction(Map<String, Object> context) {
            ...
            if (Debug.timingOn()) {
                long endTime = System.currentTimeMillis(); 
                Debug.log("Run action entityCondition in " + (endTime - startTime) + " milliseconds");
            }
        }
    }
    ...
    /**
     * Log tag. Usage: log  message="...${...} ..."
     * @author Maps 27/01/2010
     */
    public static class Log extends ModelScreenAction {
    	/**
    	 * Message string has to expand
    	 */
        protected FlexibleStringExpander message;
    	
    	public Log(ModelScreen modelScreen, Element logElement) {
			super(modelScreen, logElement);
            this.message = FlexibleStringExpander.getInstance(logElement.getAttribute("message"));
		}
    	
    	@Override
    	public void runAction(Map<String, Object> context) throws GeneralException {
   			Debug.log(this.message.expandString(context));
    	}
    }
}
