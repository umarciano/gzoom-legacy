...
(function($) {
  ...
  $.fn.expand = function() {
    ...
    childrenOf($(this)).each(function() {
      ...
      if($(this).is(".collapsed.parent")) {
        $(this).expand();
      }
      
      if($(this).is(".initially-collapsed")) {
        $(this).collapse();
      }
  ...
  //Maps added
  $.fn.hideAll = function() {
	  $(this).find("tbody tr.parent").each(function() {
		  $(this).collapse();
		  $(this).hide(); //Nascondo la root stessa
	  });
	  $(this).hide();
  };
  
  //Maps added
  $.fn.showModule = function(parentId) {
	  $(this).hideAll(); //Nascondo old items
	  if (parentId) {
		  $(this).find("tr#" + parentId +".parent").each(function() {
			  $(this).show(); //mostro la root stessa 
			  $(this).expand();					  
		  });
		  $(this).show(); //Mostro intera table (treemenu-outer-table)
	  }
  };  
	...
	if (node) {  
	    ...
	    if (classNames) {
	    	$(classNames).each( function() {
	    		if (this.match("child-of-") != null) {
	    			ret = $("#" + this.substring(9));
	    			return false;
	    		}
	    	});
	    }
