...
<script type="text/javascript">
    var WorkEffortAchieveViewList_TwoColumns_Big = {
        cellOpenManagement : function(e) {
            ...
            if (Object.isElement(cellSelected)) {
                ...
                if (managementSelectedElementItem) {
                    ...
                    managementSelectedElementItem.populateElementCollection = WorkEffortAchieveViewList_TwoColumns_Big.selectedElementPopulateCollection.curry(cellSelected);
                    
                    checkExecutability = managementSelectedElementItem.checkExecutability;
                    managementSelectedElementItem.checkExecutability = WorkEffortAchieveViewList_TwoColumns_Big.checkExecutability.curry(cellSelected);
                }
        ...
        load : function() {
            var selectableTable = $('TwoColBig_Table');
            
            selectableTable.select('td').each(function(cell) {
                Event.stopObserving(cell, 'dblclick');
                Event.observe(cell, 'dblclick', WorkEffortAchieveViewList_TwoColumns_Big.cellOpenManagement);
            });
    ...
    WorkEffortAchieveViewList_TwoColumns_Big.load();
</script>
...
<table cellspacing="15px" style="width: 100%" class="selectable dblclick-open-management" id="TwoColBig_Table">
    <tbody>
        <#assign index = 0>
        <#list workEffortAchieveList as item>
            ...
            <#if (index % 2) == 0>
                ...
                <#if !backGroundContent?has_content>
                    ...
                    <div <#if backGroundContent?has_content>class="resAnalysisWidgetImage mblShowDescription" style="color: white; height: 210px; background-image: url(<@ofbizUrl>${imgRequest}?saveView=N&imgId=${backGroundContent.dataResourceId?if_exists}</@ofbizUrl>);"<#else>class="resAnalysisWidgetBorder mblShowDescription" style="height: 210px;"</#if>>
                        <div style="text-align: center; vertical-align: middle; width: 100%; height: 80%" class="<#if showTooltip?if_exists>tooltipDescription</#if>" <#if item.description?has_content>description="${item.description}"</#if> >
