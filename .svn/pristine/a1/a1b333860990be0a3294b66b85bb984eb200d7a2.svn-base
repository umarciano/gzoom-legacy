...
if (parameters.userLogin) {
    ...
    * se nn è stato inserito nessun groupId, elimino tutte le portlet con il parent che è statio passato (verrà visualizzata solo la prima pagina),
    * altrimenti viene visualizzate le portlet dove nn ci sono  securityGroupId, o se è presente deve essere prensente anche nello user
    */
        
    userLoginSecurityGroupsVi = delegator.findList("UserLoginSecurityGroup", EntityCondition.makeCondition("userLoginId", EntityOperator.EQUALS, parameters.userLogin.userLoginId), null, null, null, false);
    
	listKeySecuriryGroup = [];
    if (UtilValidate.isNotEmpty(userLoginSecurityGroupsVi)) {
       userLoginSecurityGroupsVi.each { userLoginSecurityGroup ->
       	   listKeySecuriryGroup.add(userLoginSecurityGroup.groupId);
       }        
    }
    portalPages.each { portalPage ->
        if(UtilValidate.isEmpty(listKeySecuriryGroup)){
        	if(portalPage.parentPortalPageId.equals(parameters.parentPortalPageId)){
        		portalPages.remove(portalPage);
        	}
        }else if (UtilValidate.isNotEmpty(portalPage.securityGroupId) && listKeySecuriryGroup.contains(portalPage.securityGroupId)) {
                portalPages.remove(portalPage);
        	}
    }
