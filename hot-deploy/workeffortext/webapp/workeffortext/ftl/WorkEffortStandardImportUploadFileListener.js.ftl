...
if (!window.__weImportState) {
	window.__weImportState = { jobs: [], pendingCounter: 0 };
}
WorkEffortStandardImportUploadFileListener = {
	uploadFile : "uploadFile",
	form : false,
	jobs : window.__weImportState.jobs,
	queueId : "workeffortImportQueue",
	defaultJobType : "Caricamento Massivo Schede Performance Individuale",
	pendingCounter : 0,
	...
	renderQueue : function() {
		...
		this.jobs.forEach(function(job) {
			...
			var label = job.type || WorkEffortStandardImportUploadFileListener.defaultJobType;
	...
	addPendingJob : function(type) {
		window.__weImportState.pendingCounter += 1;
		var job = {
			type: type || this.defaultJobType,
			pendingId: "pending-" + window.__weImportState.pendingCounter,
			status: "in corso"
		};
		this.jobs.push(job);
		this.renderQueue();
	...
	pollJobStatus : function(sessionId, jobLogId) {
		...
		new Ajax.Request('<@ofbizUrl>getStandardImportStatus</@ofbizUrl>', {
			...
			onSuccess: function(transport) {
				...
				if (data.completed === 'Y') {
					WorkEffortStandardImportUploadFileListener.updateJob(sessionId, "completato");
					WorkEffortStandardImportUploadFileListener.showCompletedSummary(sessionId, data);
				} else {
					...
					setTimeout(function() {
						WorkEffortStandardImportUploadFileListener.pollJobStatus(sessionId, jobLogId);
					}, 4000);
				}
			},
			onFailure: function() {
	...
	showCompletedSummary : function(sessionId, data) {
		...
		modal_box_messages.alert(msg, null, function() {WorkEffortStandardImportUploadFileListener.refreshForm('completed')});
	},
		...
		for (var i = 0; i < this.jobs.length; i++) {
			if (this.jobs[i].sessionId === sessionId) {
				...
				if (status === "completato" || status === "errore") {
					setTimeout(function() {
						// muta l'array in place (non riassegnare): deve restare lo stesso riferimento di window.__weImportState.jobs
						var jobs = WorkEffortStandardImportUploadFileListener.jobs;
	...
	load : function() {
		...
		if (Object.isElement(form) && form.tagName === "FORM") {
			...
			console.log('[WEIMPORT-DEBUG] form registered');
		} else {
			console.log('[WEIMPORT-DEBUG] form NOT found, selector .uploadFile returned', form);
		}
	},
	...
	registerForm : function(form) {
		...
		if (Object.isElement(form)) {
			Event.observe(form, "submit", function() {
				console.log('[WEIMPORT-DEBUG] submit event fired');
				WorkEffortStandardImportUploadFileListener.addPendingJob(WorkEffortStandardImportUploadFileListener.defaultJobType);
				WorkEffortStandardImportUploadFileListener.showLoadingMessage();
				WorkEffortStandardImportUploadFileListener.notifyParentStatus('in corso', {type: WorkEffortStandardImportUploadFileListener.defaultJobType});
			...
			form.submit = function() {
				console.log('[WEIMPORT-DEBUG] form.submit() intercepted');
				WorkEffortStandardImportUploadFileListener.addPendingJob(WorkEffortStandardImportUploadFileListener.defaultJobType);
				WorkEffortStandardImportUploadFileListener.showLoadingMessage();
				WorkEffortStandardImportUploadFileListener.notifyParentStatus('in corso', {type: WorkEffortStandardImportUploadFileListener.defaultJobType});
	...
	uploadCompleted : function(event) {
		...
		if (!doc || !doc.getElementById) {
			WorkEffortStandardImportUploadFileListener.notifyParentStatus('errore', {type: WorkEffortStandardImportUploadFileListener.defaultJobType});
			return;
		}
		...
		if (sessionIdNode != null && sessionIdNode.innerHTML != "") {
			...
			WorkEffortStandardImportUploadFileListener.bindSessionToPendingJob(sessionId, uploadJobLogId);
		}
    	...
    	if (Object.isElement(errorMessageDiv)) {
			...
			if (sessionId) {
				WorkEffortStandardImportUploadFileListener.updateJob(sessionId, "errore");
			}
    	...
    	if(messTotale != ""){
  	  		modal_box_messages.alert(messTotale, null, function() {WorkEffortStandardImportUploadFileListener.refreshForm('upload')});
    	}
	},
	...
	refreshForm : function(source) {
		...
		//console.log('[WEIMPORT-DEBUG] refreshForm invocata da:', source);
		// Aggiunto passaggio del parametro 'noInfoToolbar' (fix della issue GN-5156, re-opening)
		// alert('WorkEffortStandardImportUploadFileListener.js.ftl - refreshForm, noInfoToolbar = ${parameters.noInfoToolbar?if_exists}');
		// console.log('[WorkEffortStandardImportUploadFileListener.js.ftl::refreshForm] (GN-5156) noInfoToolbar = ${parameters.noInfoToolbar?if_exists}');
