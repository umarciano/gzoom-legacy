	...
	<script type="text/javascript">
        ...
        var partyPerformanceSummaryListTable = $('table_WorkEffortPartyPerformance_${parameters.weContextId_value?if_exists}');
                    
	    if (TableKit.isSelectable(partyPerformanceSummaryListTable)) {
	        TableKit.registerObserver(partyPerformanceSummaryListTable, 'onSelectEnd', "WorkEffortPerformanceSummaryListExtension_selectEnd", function(table, e) {
	            ...
	            <#list portletAssocList as portletAssocAssoc>
	                ...
	                <#if portalPagePortletIdentifications?has_content>
	                    ...
	                    ajaxUpdateArea('portlet-container_${portalPageId}_${portalPortletId}_${portletSeqId}', '${managementPaginationTarget}','&externalLoginKey=${requestAttributes.externalLoginKey?if_exists}&weContextId_value='+weContextId_value.getValue()+'&orgUnitId='+orgUnitId.getValue()+'&workEffortIdRoot=' + workEffortId.getValue() + '&partyId='+partyId.getValue()+'&all=true&portalPageId=${portalPageId?if_exists}&portalPortletId=${portalPortletId?if_exists}&portletSeqId=${portletSeqId?if_exists}&saveView=N');
	                </#if>
	            </#list>
	        } );
	    }
    ...
    <form class="basic-form noTableResizeHeight" id="WorkEffortPartyPerformance_${parameters.weContextId_value?if_exists}" > 
     ...
     <#assign lastEnabled = parameters["VIEW_INDEX_" + parameters.PAGINATOR_NUMBER] != lastViewIndex/>
     <#if listIt?has_content && completeListSize &gt; parameters["VIEW_SIZE_" + parameters.PAGINATOR_NUMBER]>
