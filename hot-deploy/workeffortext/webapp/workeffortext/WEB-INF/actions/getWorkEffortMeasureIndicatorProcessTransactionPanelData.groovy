...
if(UtilValidate.isNotEmpty(workEffortMeasure)) {
	...
	def tp1 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
	if(UtilValidate.isNotEmpty(tp1)) {
		fromDate = tp1.fromDate;
		dayBefore = Calendar.getInstance();
		dayBefore.setTime(tp1.fromDate);
		dayBefore.add(Calendar.DATE, -1);
		def tp2 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
		if(UtilValidate.isNotEmpty(tp2)) {
			fromDate = tp2.fromDate;
			dayBefore = Calendar.getInstance();
			dayBefore.setTime(tp2.fromDate);
			dayBefore.add(Calendar.DATE, -1);
			def tp3 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
			if(UtilValidate.isNotEmpty(tp3)) {
				fromDate = tp3.fromDate;
				dayBefore = Calendar.getInstance();
				dayBefore.setTime(tp3.fromDate);
				dayBefore.add(Calendar.DATE, -1);
				def tp4 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
				if(UtilValidate.isNotEmpty(tp4)) {
					fromDate = tp4.fromDate;
					dayBefore = Calendar.getInstance();
					dayBefore.setTime(tp4.fromDate);
					dayBefore.add(Calendar.DATE, -1);
					def tp5 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
					if(UtilValidate.isNotEmpty(tp5)) {
						fromDate = tp5.fromDate;
						dayBefore = Calendar.getInstance();
						dayBefore.setTime(tp5.fromDate);
						dayBefore.add(Calendar.DATE, -1);
						def tp6 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
						if(UtilValidate.isNotEmpty(tp6)) {
							fromDate = tp6.fromDate;
							dayBefore = Calendar.getInstance();
							dayBefore.setTime(tp6.fromDate);
							dayBefore.add(Calendar.DATE, -1);
							def tp7 = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(EntityCondition.makeCondition("periodTypeId", workEffortMeasure.periodTypeId), EntityCondition.makeCondition("thruDate", dayBefore.getTime())), null, ["-thruDate"], null, false));
	...
	periodList = delegator.findList("CustomTimePeriodAndMeasureProcessView", EntityCondition.makeCondition(EntityCondition.makeCondition("workEffortMeasureId", workEffortMeasureId), EntityCondition.makeCondition("thruDate", EntityOperator.GREATER_THAN_EQUAL_TO, fromDate)), null, ["thruDate"], null, false);
