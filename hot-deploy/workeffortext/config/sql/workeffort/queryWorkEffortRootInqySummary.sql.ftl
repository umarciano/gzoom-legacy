SELECT A.ORG_UNIT_ID, G.PARTY_NAME, A.CURRENT_STATUS_ID, U.ULVR_USER_LOGIN_ID AS UV_USER_LOGIN_ID, C.DESCRIPTION AS WE_STATUS_DESCR, C.DESCRIPTION_LANG AS WE_STATUS_DESCR_LANG, C.ACT_ST_ENUM_ID AS WE_ACTIVATION, 
  ...
  B.PARENT_TYPE_ID AS WE_CONTEXT_ID, A.WORK_EFFORT_SNAPSHOT_ID, G.PARTY_NAME_LANG, PPR.PARENT_ROLE_CODE, G.EXTERNAL_ID, A.ORGANIZATION_ID
...
LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> E ON (A.ORG_UNIT_ROLE_TYPE_ID = E.ROLE_TYPE_ID_FROM 
...
LEFT OUTER JOIN <@table "WORK_EFFORT_PARTY_ASSIGNMENT"/> F ON (A.WORK_EFFORT_ID = F.WORK_EFFORT_ID AND F.ROLE_TYPE_ID LIKE 'WEM%' 
  AND F.PARTY_ID = U.UL_PARTY_ID 
  AND F.FROM_DATE <= A.ESTIMATED_COMPLETION_DATE 
  AND (F.THRU_DATE IS NULL OR F.THRU_DATE >= A.ESTIMATED_COMPLETION_DATE)) 
...
GROUP BY A.ORG_UNIT_ID, G.PARTY_NAME, A.CURRENT_STATUS_ID, U.ULVR_USER_LOGIN_ID, C.DESCRIPTION, C.DESCRIPTION_LANG, C.ACT_ST_ENUM_ID, C.SEQUENCE_ID, C.STATUS_TYPE_ID, D.MANAGEMENT_ROLE_TYPE_ID, D.MANAG_WE_STATUS_ENUM_ID, E.PARTY_ID_TO, 
  F.PARTY_ID, B.PARENT_TYPE_ID, A.WORK_EFFORT_SNAPSHOT_ID, G.PARTY_NAME, G.PARTY_NAME_LANG, PPR.PARENT_ROLE_CODE, G.EXTERNAL_ID, A.ORGANIZATION_ID
