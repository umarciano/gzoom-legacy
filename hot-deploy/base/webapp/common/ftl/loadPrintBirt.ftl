<#if listIt?has_content>
    ...
    <#list listIt as print>   
        ...
        <p><input type="radio" class="print-radio" name="reportContentId" value="${print.contentId}" onClick="radioOnChange.load(this.getValue())" <#if index=0> checked=true </#if>/><span class="readonly">${print.etch?default(print.description)}</span> </p>
        <#assign index = index+1>  		
    </#list>    
</#if>
