...
CREATE OR REPLACE
	VIEW RELATIONSHIP_ORG_VIEW AS SELECT
		...
		FROM_DATE,
		CASE
			WHEN THRU_DATE IS NULL THEN TO_DATE(
				'2099-12-31',
				'YYYY-MM-DD'
			)
			ELSE THRU_DATE
...
UNION SELECT
		TD.PARTY_ID,
		TD.FROM_DATE - 1 AS THRU_DATE
	FROM
		RELATIONSHIP_FROM_DATE_VIEW TD -- ORACLE
	WHERE
		(
			EXISTS(
				SELECT
...
UNION SELECT
		TD.PARTY_ID,
		TD.THRU_DATE + 1 AS FROM_DATE
	FROM
		RELATIONSHIP_THRU_DATE_VIEW TD -- ORACLE
	WHERE
		(
			EXISTS(
				SELECT
