...
if(UtilValidate.isNotEmpty(userLoginSecurityGroupList)) {
	userLoginSecurityGroupList.each{ userLoginSecurityGroupItem ->
		def noPortalGdprConditions = [];
		noPortalGdprConditions.add(EntityCondition.makeCondition("userLoginId", userLoginSecurityGroupItem.userLoginId));
		noPortalGdprConditions.add(EntityCondition.makeCondition("groupId", "NOPORTAL_GDPR"));
		def noPortalGdprList = delegator.findList("UserLoginSecurityGroup", EntityCondition.makeCondition(noPortalGdprConditions), null, null, null, false);
	
	    if (UtilValidate.isEmpty(noPortalGdprList)) {
	    	...
	    	userLoginSecurityGroup.put("groupId", "NOPORTAL_GDPR");
...
if(UtilValidate.isNotEmpty(securityGroupList)) {
	securityGroupList.each{ securityGroupItem ->
	    ...
	    if (UtilValidate.isNotEmpty(userLoginSecurityGroups)) {
			...
			securityGroupContentConditions.add(EntityCondition.makeCondition("contentId", "NOPORTAL_GDPR"));
		    ...
		    if (UtilValidate.isEmpty(securityGroupContentList)) {
		    	...
		    	securityGroupContent.put("contentId", "NOPORTAL_GDPR");
