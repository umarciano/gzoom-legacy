...
conditionReminder = EntityCondition.makeCondition(EntityCondition.makeCondition("jobName", "Sollecito Stampa Obiettivi Individuali"),
    EntityCondition.makeCondition("statusId",  "SERVICE_PENDING")
);
...
if (UtilValidate.isEmpty(reminderScheduled) || reminderScheduled.size() <= 0) {
    ...
    jobSandBoxReminder.put("jobId", "REMINDER_ASS_OBI");
    jobSandBoxReminder.put("jobName", "Sollecito Stampa Obiettivi Individuali");
    jobSandBoxReminder.put("runTime", new Timestamp(UtilDateTime.toDate(1, 1, 2000, 0, 0, 0).getTime()));
    jobSandBoxReminder.put("serviceName", "reminderAssObiScheduled");
