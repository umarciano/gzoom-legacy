...
conditionReminder = EntityCondition.makeCondition(EntityCondition.makeCondition("jobName", "Sollecito Scadenza"),
    EntityCondition.makeCondition("statusId",  "SERVICE_PENDING")
);
...
if (UtilValidate.isEmpty(reminderScheduled) || reminderScheduled.size() <= 0) {
    ...
    jobSandBoxReminder.put("jobId", "REMINDER_SCADENZA");
    jobSandBoxReminder.put("jobName", "Sollecito Scadenza");
    jobSandBoxReminder.put("runTime", new Timestamp(UtilDateTime.toDate(1, 1, 2000, 0, 0, 0).getTime()));
    jobSandBoxReminder.put("serviceName", "reminderScadObiScheduled");
